        ...
        case 6:
            {
                ...
                return Coefficient(z) * build_ppl_Linear_Expression(Field(e,
                                                                          1));
            }

        default:
            throw std::invalid_argument(
                      "PPL OCaml interface invalid_argument\n:"
...
build_ppl_Constraint (value c)
{
    ...
    switch (Tag_val(c))
    {
        case 0:
            // Less_Than
            return build_ppl_Linear_Expression(e1) <
                   build_ppl_Linear_Expression(e2);
        ...
        default:
            throw std::invalid_argument(
                      "PPL OCaml interface invalid argument:\n"
        ...
        case Generator::LINE:
            {
                ...
                // allocate one block (the linear expression) with Tag 0 (a line
                // here).
                caml_generator = caml_alloc(1, 0);
                Store_field(caml_generator, 0, get_linear_expression(
                                ppl_generator));
        ...
        case Generator::POINT:
            {
                ...
                Store_field(caml_generator, 1, build_ocaml_coefficient(
                                divisor));
                CAMLreturn(caml_generator);
            }
        ...
        case Constraint::EQUALITY:
            {
                ...
                Store_field(caml_constraint, 0, get_linear_expression(
                                ppl_constraint));
                Store_field(caml_constraint, 1, get_inhomogeneous_term(
                                ppl_constraint));
        ...
        case 2:
            {
                ...
                return Generator::point(build_ppl_Linear_Expression(Field(g,
                                                                          0)),
                                        Coefficient(z));
            }
        ...
        case 3:
            {
                ...
                return Generator::closure_point(build_ppl_Linear_Expression(
                                                    Field(g, 0)),
                                                Coefficient(z));
            }
                     ...
                     value caml_opt_mode) try
{
    ...
    const Linear_Expression ppl_cost = build_ppl_Linear_Expression(
        caml_cost);
    const Optimization_Mode ppl_opt_mode = build_ppl_opt_mode(caml_opt_mode);
    MIP_Problem           & ppl_mip      = *new MIP_Problem(dd, ppl_cs,
                                                            ppl_cost,
                                                            ppl_opt_mode);
                                                 ...
                                                 value caml_ivars) try
{
    ...
    ppl_mip.add_to_integer_space_dimensions(build_ppl_Variables_Set(
                                                caml_ivars));
    CAMLreturn(Val_unit);
}
CATCH_ALL
                                                   ...
                                                   value caml_ivars) try
{
    ...
    ppl_pip.add_to_parameter_space_dimensions(build_ppl_Variables_Set(
                                                  caml_ivars));
    CAMLreturn(Val_unit);
}
CATCH_ALL
...
ppl_PIP_Tree_Node_true_child (value caml_node) try
{
    ...
    const PIP_Tree_Node * ppl_node = p_PIP_Tree_Node_val(caml_node);
    const PIP_Tree_Node * child    = ppl_PIP_Tree_Node_get_child(ppl_node,
                                                                 true);
...
ppl_set_timeout (value time) try
{
    ...
    p_timeout_object = new Watchdog(cpp_time, abandon_expensive_computations,
                                    e);
    CAMLreturn(Val_unit);
}
CATCH_ALL
...
ppl_set_deterministic_timeout (value unscaled_weight, value scale) try
{
    ...
    unsigned cpp_scale =
        value_to_unsigned<unsigned>(scale);
