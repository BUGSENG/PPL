...
handle_exception (const not_a_nil_terminated_list & e)
{
    ...
    Prolog_put_nil(expected);
    Prolog_construct_cons(expected,
                          Prolog_atom_term_from_string("Prolog_list"),
                          expected);
    Prolog_construct_compound(expected, a_expected, expected);
...
build_congruence (Prolog_term_ref t, const char * where)
{
    if (Prolog_is_compound(t))
    {
        ...
        if (arity == 2)
        {
            ...
            else if (functor == a_is_congruent_to)
            {
                // =:=
                if (Prolog_is_integer(arg2))
                {
    ...
    if (Prolog_is_compound(t))
    {
        ...
        else if (arity == 2)
        {
            ...
            if (Prolog_is_integer(arg2))
            {
                ...
                {
                    return Generator::closure_point(build_linear_expression(
                                                        arg1, where),
                                                    integer_term_to_Coefficient(
                                                        arg2));
                }
...
build_grid_generator (Prolog_term_ref t, const char * where)
{
    if (Prolog_is_compound(t))
    {
        ...
        if (arity == 1)
        {
            ...
            {
                return Grid_Generator::grid_line(build_linear_expression(arg,
                                                                         where));
            }
            else if (functor == a_parameter)
            {
                return Grid_Generator::parameter(build_linear_expression(arg,
                                                                         where));
            }
            else if (functor == a_grid_point)
            {
                return Grid_Generator::grid_point(build_linear_expression(arg,
                                                                          where));
            }
        }
        else if (arity == 2)
        {
            ...
            if (Prolog_is_integer(arg2))
            {
                ...
                {
                    return Grid_Generator::grid_point(build_linear_expression(
                                                          arg1,
                                                          where),
                                                      integer_term_to_Coefficient(
                                                          arg2));
                ...
                {
                    return Grid_Generator::parameter(build_linear_expression(
                                                         arg1,
                                                         where),
                                                     integer_term_to_Coefficient(
                                                         arg2));
...
congruence_term (const Congruence & cg)
{
    ...
    Prolog_construct_compound(t_tmp,
                              relation1,
                              get_homogeneous_expression(cg),
                              Coefficient_to_integer_term(
                                  -cg.inhomogeneous_term()));
    Prolog_construct_compound(t,
                              relation2,
                              t_tmp,
...
term_to_Variable (Prolog_term_ref t, const char * where)
{
    if (Prolog_is_compound(t))
    {
        ...
        if ((functor == a_dollar_VAR) && (arity == 1))
        {
            ...
            Prolog_get_arg(1, t, arg);
            return
                Variable(term_to_unsigned<dimension_type>(arg,
                                                          "term_to_Variable"));
        }
    }
...
Prolog_atom
term_to_pip_problem_control_parameter_name (Prolog_term_ref t, const
                                            char * where)
{
    if (Prolog_is_atom(t))
    {
                  ...
                  Coefficient & n, Coefficient & d)
{
    ...
    else if (Prolog_is_atom(t_limit))
    {
        ...
        Prolog_atom allowed_infinity = (kind == LOWER_BOUNDARY ? a_minf :
                                        a_pinf);
...
term_to_complexity_class (Prolog_term_ref t, const char * where)
{
    if (Prolog_is_atom(t))
    {
        ...
        if (Prolog_get_atom_name(t, &name)
            && ((name == a_polynomial) || (name == a_simplex) || (name ==
                                                                  a_any)))
        {
            return name;
        }
...
ppl_max_space_dimension (Prolog_term_ref t_msd)
{
    try
    {
        if (unify_ulong(t_msd, max_representable_dimension(
                            max_space_dimension())))
        {
            return PROLOG_SUCCESS;
        }
...
ppl_initialize ()
{
    try
    {
        ...
        {
            Prolog_atom a = Prolog_atom_from_string(
                prolog_interface_atoms[i].name);
...
ppl_set_timeout (Prolog_term_ref t_csecs)
{
    try
    {
        ...
        unsigned                 csecs = term_to_unsigned<unsigned>(t_csecs,
                                                                    "ppl_set_timeout/1");
                               ...
                               Prolog_term_ref t_scale)
{
    try
    {
        ...
        unsigned scale
            = term_to_unsigned<unsigned>(t_scale,
                                         "ppl_set_deterministic_timeout/2");
...
extern "C" Prolog_foreign_return_type
ppl_new_MIP_Problem_from_space_dimension (Prolog_term_ref t_nd, Prolog_term_ref
                                          t_mip)
{
                     ...
                     Prolog_term_ref t_mip)
{
    ...
    try
    {
        ...
        Optimization_Mode mode = (opt == a_max) ? MAXIMIZATION :
                                 MINIMIZATION;
...
extern "C" Prolog_foreign_return_type
ppl_MIP_Problem_add_space_dimensions_and_embed (Prolog_term_ref t_mip,
                                       ...
                                       Prolog_term_ref t_cp_value)
{
    ...
    try
    {
        ...
        Prolog_atom cp_value = term_to_control_parameter_value(t_cp_value,
                                                               where);

        if (cp_value == a_pricing_steepest_edge_float)
        {
            mip->set_control_parameter(
                                       ...
                                       Prolog_term_ref t_cp_value)
{
    ...
    try
    {
        ...
        Prolog_atom cp_name =
            term_to_control_parameter_name(t_cp_name, where);
...
extern "C" Prolog_foreign_return_type
ppl_new_PIP_Problem_from_space_dimension (Prolog_term_ref t_nd, Prolog_term_ref
                                          t_pip)
{
                     ...
                     Prolog_term_ref t_pip)
{
    ...
    try
    {
        ...
        PIP_Problem * pip = new PIP_Problem(dim, cs.begin(), cs.end(),
                                            params);
        Prolog_term_ref tmp = Prolog_new_term_ref();
...
extern "C" Prolog_foreign_return_type
ppl_PIP_Problem_add_space_dimensions_and_embed (Prolog_term_ref t_pip,
                                                ...
                                                Prolog_term_ref t_num_params)
{
    ...
    try
    {
        ...
        dimension_type np = term_to_unsigned<dimension_type>(t_num_params,
                                                             where);
                                       ...
                                       Prolog_term_ref t_cp_value)
{
    ...
    try
    {
        ...
        Prolog_atom cp_name =
            term_to_pip_problem_control_parameter_name(t_cp_name, where);
                                       ...
                                       Prolog_term_ref t_cp_value)
{
    ...
    try
    {
        ...
        Prolog_atom cp_value = term_to_pip_problem_control_parameter_value(
            t_cp_value, where);
                                     ...
                                     Prolog_term_ref t_pip_tree)
{
    ...
    try
    {
        ...
        PIP_Tree_Node * sol =
            const_cast<PIP_Tree_Node *>(pip->optimizing_solution());
        Prolog_term_ref t_sol = Prolog_new_term_ref();
                                     ...
                                     Prolog_term_ref t_le)
{
    ...
    try
    {
        ...
        if (Prolog_unify(t_le, get_linear_expression(pip->parametric_values(
                                                         var))))
        {
            return PROLOG_SUCCESS;
        }
                              ...
                              Prolog_term_ref t_ptree)
{
    ...
    try
    {
        ...
        bool            b         = true;
        PIP_Tree_Node * ppl_ptree =
            const_cast<PIP_Tree_Node *>(pip->child_node(b));
