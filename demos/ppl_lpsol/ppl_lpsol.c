...
#ifdef PPL_HAVE_GETOPT_H
        option_index = 0;
        c            = getopt_long(argc, argv, OPTION_LETTERS, long_options,
                                   &option_index);
#else
        ...
        switch (c)
        {
            ...
            case 'c':
                ...
                if (optarg)
                {
                    ...
                    {
                        fatal(
                            "only a non-negative floating point number can `-c'");
                    }
                    else
                    {
                        check_threshold = d;
                     ...
                     const double ppl_optimum_value)
{
             ...
             && (ppl_status == PPL_MIP_PROBLEM_STATUS_OPTIMIZED))
    {
        ...
        double glpk_optimum_value
            = (treat_as_lp ? glp_get_obj_val(glpk_lp) : glp_mip_obj_val(
                   glpk_lp));
                    ...
                    ppl_Generator_t   point)
{
    ...
    if (!no_mip)
    {
        ppl_MIP_Problem_add_to_integer_space_dimensions(ppl_mip,
                                                        args->integer_variables,
                                                        args->
    ...
    if (print_timings)
    {
        fprintf(stderr, "Time to solve the problem %s: ",
                args->input_file_name);
        print_clock(stderr, &args->saved_ru_utime);
        fprintf(stderr, " s\n");
        start_clock(&args->saved_ru_utime);
...
process_args (void * vp)
{
...
#ifndef NDEBUG
    ppl_new_Constraint_System_from_Constraint_System(&ppl_cs_copy,
                                                     args->ppl_cs);
#endif
    ...
    if (optimum_found)
    {
...
#ifndef NDEBUG
        {
            ...
            relation = ppl_Polyhedron_relation_with_Generator(ph,
                                                              optimum_location);
...
prepare_args (const char * file_name)
{
    ...
    if ((num_integer_variables > 0) && !no_mip && use_simplex)
    {
        ...
        integer_variables = (ppl_dimension_type *)
                            malloc((num_integer_variables + 1) *
    ...
    for (row = 1; row <= num_rows; ++row)
    {
        ...
        nz = glp_get_mat_row(glpk_lp, row, coefficient_index,
                             coefficient_value);

        for (i = 1; i <= nz; ++i)
        {
            set_mpq_t_from_double(rational_coefficient[i],
                                  coefficient_value[i]);
            /* Update den_lcm. */
        ...
        for (i = 1; i <= nz; ++i)
        {
            ...
            ppl_assign_Coefficient_from_mpz_t(ppl_coeff, tmp_z);
            ppl_Linear_Expression_add_to_coefficient(ppl_le,
                                                     coefficient_index[i] - 1,
                                                     ppl_coeff);
        }

        type = glp_get_row_type(glpk_lp, row);
        add_constraints(ppl_le, type, rational_lb, rational_ub, den_lcm,
                        ppl_cs);
    ...
    for (i = 1; i <= dimension; ++i)
    {
        ...
        ppl_assign_Coefficient_from_mpz_t(ppl_coeff, tmp_z);
        ppl_Linear_Expression_add_to_coefficient(ppl_objective_le, i - 1,
                                                 ppl_coeff);
    }
...
main (int argc, char * argv[])
{
    ...
    extern void _glp_lib_print_hook(int ( * func )(void * info, const
                                                   char * buf),
                                    void * info);
#endif
