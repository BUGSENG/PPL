...
#    error \
    "Exactly one among USE_PPL, USE_POLKA and USE_POLYLIB must be defined"
#endif
...
#ifdef PPL_HAVE_GETOPT_H
        int       option_index = 0;
        const int c            = getopt_long(argc, argv, OPTION_LETTERS,
                                             long_options,
                                             &option_index);
#else
            ...
            case 'R':
                {
                    ...
                    {
                        max_bytes_of_virtual_memory = static_cast<unsigned
                                                                  long>(l) *
                                                      MEGA;
                    }
                }
...
#    endif
    ...
    {
        // If no input files have been specified: we will read from standard
        // input.
...
read_polyhedron (std::istream & in, POLYHEDRON_TYPE & ph)
{
    ...
    while (true)
    {
        ...
        else if ((s == "linearity") || (s == "equality") || (s ==
                                                             "partial_enum"))
        {
    ...
    if (rep == V)
    {
        ...
        for (row = 0; !has_num_rows || row < num_rows; ++row)
        {
            ...
            if (vertex_marker == 1)
            {
                ...
                value_assign(mat->p[row][space_dim + 1],
                             denominator.get_mpz_t());
#endif
                has_a_point = true;
            }
            else if (linearity.find(row + 1) != linearity_end)
            {
                ...
                // and the common denominator at index space_dim+1 (0 for
                // ray/line).
        ...
        if (verbose)
        {
            if (!has_num_rows)
            {
                std::cerr << "Problem dimension: " << row << " x " <<
                    num_columns
                          << "; number type: " << s
                          << std::endl;
            }
    ...
    else
    {
        ...
        for (row = 0; !has_num_rows || row < num_rows; ++row)
        {
            ...
            if (linearity.find(row + 1) != linearity_end)
            {
                ...
                cs.insert(e == 0);
#elif defined(USE_POLKA)
                // NewPolka stores the constraint kind at index 0 (0 =
                // equality).
                pkint_set_si(mat->p[row][0], 0);
#elif defined(USE_POLYLIB)
            ...
            else
            {
                ...
                cs.insert(e >= 0);
#elif defined(USE_POLKA)
                // NewPolka stores the constraint kind at index 0 (1 =
                // inequality).
                pkint_set_si(mat->p[row][0], 1);
#elif defined(USE_POLYLIB)
                // PolyLib stores the constraint kind at index 0 (1 =
                // inequality).
                value_set_si(mat->p[row][0], 1);
#endif
            }
            ...
            case H_to_V:
                {
                    ...
                    {
                        warning(
                            "checking an H-to-V conversion with an H representation");
                    }

                    // Count the number of generators of `ph'.
                    unsigned                      ph_num_generators = 0;
                    const PPL::Generator_System & ph_gs             =
                        ph.generators();

                    for (PPL::Generator_System::const_iterator i =
                             ph_gs.begin(),
                    ...
                    const PPL::Generator_System & e_ph_gs             =
                        e_ph.generators();
                    ...
                    else if (ph_num_generators != e_ph_num_generators)
                    {
                        ...
                        std::cerr <<
                            "Check failed: different number of generators:\n"
            ...
            case V_to_H:
                {
                    ...
                    {
                        warning(
                            "checking an V-to-H conversion with a V representation");
                    }

                    // Count the number of constraints of `ph'.
                    unsigned                       ph_num_constraints = 0;
                    const PPL::Constraint_System & ph_cs              =
                        ph.constraints();

                    for (PPL::Constraint_System::const_iterator i =
                             ph_cs.begin(),
                    ...
                    const PPL::Constraint_System & e_ph_cs              =
                        e_ph.constraints();
                    ...
                    else if (ph_num_constraints != e_ph_num_constraints)
                    {
                        ...
                        std::cerr <<
                            "Check failed: different number of constraints:\n"
