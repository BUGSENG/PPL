                 ...
                 const int                                      indent = 0)
{
    ...
    else
    {
        ...
        const PPL::PIP_Decision_Node * const decision_node_p =
            pip->as_decision();
        ...
        {
            const PPL::PIP_Solution_Node * const solution_node_p =
                pip->as_solution();
...
process_options (const int argc, char * const argv[])
{
    while (true)
    {
        ...
        switch (c)
        {
            ...
            case 'M':
                pivot_row_strategy =
                    PPL::PIP_Problem::PIVOT_ROW_STRATEGY_MAX_COLUMN;
            break;
...
{
    fatal(
        "internal error: please submit a bug report to ppl-devel@cs.unipr.it");
    exit(1);
}
