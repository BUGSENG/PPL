...
PPL::Polyhedron::initialize ()
{
    ...
    simplify_num_saturators_p =
        new dimension_type[simplify_num_saturators_size];
}
...
PPL::Polyhedron::relation_with (const Congruence & cg) const
{
    ...
    // Build the equality corresponding to the congruence (ignoring the
    // modulus).
...
PPL::Polyhedron::is_universe () const
{
    ...
    if (has_pending_generators())
    {
        ...
        if (num_lines + num_pending_lines < space_dim)
        {
            ...
            // In order to span an n dimensional space (where n =
            // num_dims_missing),
    ...
    else
    {
        ...
        else
        {
            ...
            PPL_ASSERT(
                eps_leq_one.expression().all_homogeneous_terms_are_zero());
            PPL_ASSERT(
                eps_geq_zero.expression().all_homogeneous_terms_are_zero());
#endif
            return true;
        }
    }
...
PPL::Polyhedron::contains_integer_point () const
{
         ...
         cs_end = cs.end(); cs_i != cs_end; ++cs_i)
    {
        ...
        if (c_type == Constraint::STRICT_INEQUALITY)
        {
            ...
            {
                // NOTE: since tautological constraints are already filtered
                // away
        ...
        else
        {
            ...
            else
            {
                ...
                // to see whether or not the inhomogeneous term can be
                // tightened.
                homogeneous_gcd = c.expression().gcd(1, space_dim + 1);
                ...
                if (homogeneous_gcd == 0)
                {
                    // NOTE: since tautological constraints are already filtered
                    // away
                    // by iterators, here we must have an inconsistent
                    // constraint.
                    PPL_ASSERT(c.is_inconsistent());
                    return false;
                }
            ...
            if (gen_sys_i.is_line_or_ray())
            {
                ...
                if (sign != 0)
                {
                    if (gen_sys_i.expression().all_zeroes(1, var_id)
                        && gen_sys_i.expression().all_zeroes(var_id + 1,
                                                             space_dim + 1))
                    {
...
#    endif
        ...
        if (generators_are_minimized())
        {
            ...
            gs_without_pending.remove_trailing_rows(
                gs_without_pending.num_rows()
            ...
            if ((gs_without_pending.num_rows() != copy_of_gen_sys.num_rows())
                ...
                || (gs_without_pending.num_rays() !=
                    copy_of_gen_sys.num_rays()))
            {
#ifndef NDEBUG
    ...
    if (sat_c_is_up_to_date())
    {
        for (dimension_type i = sat_c.num_rows(); i-- > 0; )
        {
            ...
            {
                const bool sat_j = (Scalar_Products::sign(con_sys[j],
                                                          tmp_gen) == 0);
    ...
    if (sat_g_is_up_to_date())
    {
        for (dimension_type i = sat_g.num_rows(); i-- > 0; )
        {
            ...
            {
                const bool sat_j = (Scalar_Products::sign(tmp_con,
                                                          gen_sys[j]) == 0);
        ...
        else
        {
            ...
            switch (g.type())
            {
                ...
                case Generator::POINT:
                    ...
                    if (has_pending)
                    {
                        gen_sys.insert_pending(Generator::point(nc_expr,
                                                                g.divisor()));
                    }
                    else
                    {
...
PPL::Polyhedron::simplify_using_context_assign (const Polyhedron & y)
{
    ...
    if (!x.minimize())
    {
        ...
        PPL_ASSERT(!y.has_pending_generators() &&
                   y.constraints_are_up_to_date());
        ...
        if (!z.minimize())
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    lp.add_constraints(y_cs);
                    const_cast<Constraint_System &>(y_cs).
            ...
            // the number of generators of `y' they rule out (the more
            // generators
                ...
                if (!redundant_by_y[i])
                {
                         ...
                         y_gs_end = y_gs.end(); k != y_gs_end; ++k)
                    {
                        ...
                        if (x.is_necessarily_closed())
                        {
                            ...
                            {
                                // `g' is either a ray, a point or a closure
                                // point.
                                if (c.is_inequality())
                                {
                        ...
                        else
                        {
                            ...
                            switch (g.type())
                            {
                                ...
                                case Generator::POINT:
                                    ...
                                    // Have to perform the special test when
                                    // dealing with
                                    // a strict inequality.
                                    switch (c.type())
                                    {
                                ...
                                case Generator::CLOSURE_POINT:
                                    ...
                                    if (c.is_inequality())
                                    {
                                        // Constraint `c' is either a strict or
                                        // a non-strict
                                        // inequality.
                                        if (sp_sign < 0)
                                        {
...
ruled_out:
                 ...
                 ++j)
            {
                ...
                else
                {
                    ...
                    // closed, so that we can interpret the epsilon dimension as
                    // a
                    // standard dimension.  Be careful to reset the topology of
                    // `c'
        ...
        else
        {
            ...
            if (needed_non_redundant_eq > 0)
            {
                ...
                // Try to find another `needed_non_redundant_eq' linear
                // independent
            ...
            {
                const Constraint & non_redundant_ineq_i =
                    *(non_redundant_ineq_p[i]);
                Bit_Row & sat_i = sat[i];
                ...
                if (sat_i.empty() && (num_non_redundant_eq <
                                      needed_non_redundant_eq))
                {
                    // `non_redundant_ineq_i' is actually masking an equality
                    // and we are still looking for some masked inequalities.
                    // Iteration goes downwards, so the inequality comes from
                    // x_cs.
                    ...
                    {
                        // It is independent: add the _inequality_ to
                        // non_redundant_eq.
    ...
    if (expr.coefficient(var) != 0)
    {
        ...
        if (generators_are_up_to_date())
        {
            ...
            else
            {
                ...
                // we negate everything once more, as
        ...
        case GREATER_THAN:
            {
                ...
                // We split each point of the generator system into two
                // generators:
                // a closure point, having the same coordinates of the given
                // point,
                // and another point, having the same coordinates for all but
                // the
                // `var' dimension, which is displaced along the direction of
                // the
                ...
                for (dimension_type i = gen_sys.num_rows(); i-- > 0; )
                {
                    ...
                    if (gen_i.is_point())
                    {
                        ...
                        // Note: copying is really meant, to avoid undefined
                        // behavior.
                        gen_sys.sys.rows.push_back(Generator(gen_i));

                        // Note: (re-)compute references (invalidated by
                        // push_back).
                        ...
                        // Displace `new_gen' by `var' (i.e., along the new
                        // ray).
                            ...
                            Coefficient & val_n, Coefficient & val_d) const
{
    ...
    for (dimension_type i = gen_sys.num_rows(); i-- > 0; )
    {
        ...
        else
        {
            ...
            assign_r(candidate.get_den(), gen_sys_i.expr.inhomogeneous_term(),
                     ROUND_NOT_NEEDED);
