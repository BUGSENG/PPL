...
template <typename C>
typename Enable_If<Is_Same_Or_Derived<I_Constraint_Base, C>::value,
                   I_Result>::type
Interval<Boundary, Info>::lower_extend (const C & c)
{
    ...
    min_assign(LOWER, lower(), info(), LOWER, c.value(), f_info(c.value(),
                                                                open));
    PPL_ASSERT(OK());
    return I_ANY;
}
...
Interval<Boundary, Info>::upper_extend (const C & c)
{
    ...
    max_assign(UPPER, upper(), info(), UPPER, c.value(), f_info(c.value(),
                                                                open));
    PPL_ASSERT(OK());
    return I_ANY;
}
...
Interval<Boundary, Info>::Interval (const char * s)
{
    ...
    switch (lower_r)
    {
        ...
        case V_LT_PLUS_INFINITY:
            ...
            if ((upper_r == V_EQ_PLUS_INFINITY) || (upper_r ==
                                                    V_LT_PLUS_INFINITY))
            {
                assign(UNIVERSE);
            }
    ...
    switch (upper_r)
    {
        ...
        case V_GT_MINUS_INFINITY:
            ...
            if ((lower_r == V_EQ_MINUS_INFINITY) || (lower_r ==
                                                     V_GT_MINUS_INFINITY))
            {
                assign(UNIVERSE);
            }
