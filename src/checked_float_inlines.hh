...
{
    if (To_Policy::check_fpu_nan_result && is_nan<From_Policy>(from))
    {
...
assign_float_float (To & to, const From from, Rounding_Dir dir)
{
    ...
    {
        return assign_float_float_inexact<To_Policy, From_Policy>(to, from,
                                                                  dir);
    }
    else
    {
...
add_float (Type & to, const Type x, const Type y, Rounding_Dir dir)
{
    ...
    if (To_Policy::check_fpu_nan_result && is_nan<To_Policy>(to))
    {
        return V_NAN;
    }
...
{
    if (To_Policy::check_fpu_nan_result && is_nan<From_Policy>(x))
    {
        return assign_special<To_Policy>(to, VC_NAN, ROUND_IGNORE);
    }
...
assign_float_mpz (T & to, const mpz_class & from, Rounding_Dir dir)
{
    ...
    {
        mpz_mul_2exp(mantissa, from_z, Float<T>::Binary::MANTISSA_BITS -
                     exponent);
    }
