...
::difference_assign (const Pointset_Powerset & y)
{
    ...
    for (const_iterator yi = y.begin(), y_end = y.end(); yi != y_end; ++yi)
    {
             ...
             ns_end = new_sequence.end(); itr != ns_end; ++itr)
        {
            ...
            const Pointset_Powerset<NNC_Polyhedron> & residues =
                partition.second;

            // Append the contents of `residues' to `tmp_sequence'.
            std::copy(residues.begin(), residues.end(), back_inserter(
                          tmp_sequence));
        }
                        ...
                        const Pointset_Powerset<NNC_Polyhedron> & ps)
{
         ...
         i = ps.begin(), ps_end = ps.end(); i != ps_end; ++i)
    {
        ...
        {
            Pointset_Powerset<NNC_Polyhedron> new_disjuncts(
                ph.space_dimension(),
                EMPTY);
    ...
    for (const_iterator yi = y.begin(), y_end = y.end(); yi != y_end; ++yi)
    {
             ...
             ns_end = new_sequence.end(); itr != ns_end; ++itr)
        {
            ...
            const std::pair<Grid, Pointset_Powerset<Grid> > partition
                = approximate_partition(gr_yi, itr->pointset(),
                                        finite_partition);
    ...
    : Base(), space_dim(y.space_dimension())
{
    ...
    {
        x.sequence.push_back(Determinate<NNC_Polyhedron>(NNC_Polyhedron(
                                                             i->pointset())));
    }
    ...
    : Base(), space_dim(y.space_dimension())
{
    ...
    for (Pointset_Powerset<NNC_Polyhedron>::const_iterator i = y.begin(),
         y_end = y.end(); i != y_end; ++i)
    {
        x.sequence.push_back(Determinate<C_Polyhedron>(C_Polyhedron(
                                                           i->pointset())));
    }
