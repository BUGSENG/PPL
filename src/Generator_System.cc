...
PPL::Generator_System::relation_with (const Constraint & c) const
{
    ...
    switch (c.type())
    {
        ...
        case Constraint::EQUALITY:
            {
                ...
                // The following integer variable will hold the scalar product
                // sign
                // of either the first point or the first non-saturating ray we
                // find.
                ...
                for (dimension_type i = n_rows; i-- > 0; )
                {
                    ...
                    if (sp_sign == 0)
                    {
                        if (g.is_point())
                        {
                            ...
                            {
                                // We already found a point or a non-saturating
                                // ray.
                    ...
                    else
                    {
                        ...
                        switch (g.type())
                        {
                            ...
                            case Generator::LINE:
                                // If a line does not saturate `c', then there
                                // is a strict
                                // intersection between the points satisfying
                                // `c'
                            ...
                            case Generator::RAY:
                                ...
                                if (first_point_or_nonsaturating_ray_sign == 2)
                                {
                                    // It is the first time that we have a
                                    // non-saturating ray
                                    // and we have not found any point yet.
                                    first_point_or_nonsaturating_ray_sign =
                                        sp_sign;
                                    result =
                            ...
                            case Generator::CLOSURE_POINT:
                                ...
                                // NOTE: a non-saturating closure point is
                                // treated as
                                // a normal point.
                                if (first_point_or_nonsaturating_ray_sign == 2)
                                {
                                    // It is the first time that we find a point
                                    // and
                                    // we have not found a non-saturating ray
                                    // yet.
                                    first_point_or_nonsaturating_ray_sign =
                                        sp_sign;
                                    result =
        ...
        case Constraint::NONSTRICT_INEQUALITY:
            {
                // The hyperplane implicitly defined by the non-strict
                // inequality `c'
                ...
                for (dimension_type i = n_rows; i-- > 0; )
                {
                    ...
                    // inequality. If that is the case, then we have to do
                    // something
                    ...
                    if (sp_sign == 0)
                    {
                        if (g.is_point())
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // Since g saturates c, we have a strict
                                    // intersection if
                                    // none of the generators seen so far are
                                    // included in `c'.
                                    return Poly_Con_Relation::
                                           strictly_intersects();
                                }
                ...
                : Poly_Con_Relation::is_disjoint();
                                ...
                                else
                                {
                                    ...
                                    if (((sp_sign > 0)
                                        ...
                                        || ((sp_sign < 0)
                                            && result.implies(
                                                Poly_Con_Relation::is_included())))
                                    {
                                        // We have a strict intersection if
                                        // either:
                                        // - `g' satisfies `c' but none of the
                                        // generators seen
                                        //    so far are included in `c'; or
                                        // - `g' does not satisfy `c' and all
                                        // the generators
                                    ...
                                    if (sp_sign > 0)
                                    {
                                        // Here all the generators seen so far
                                        // either saturate
                            ...
                            case Generator::CLOSURE_POINT:
                                ...
                                if (first_point_or_nonsaturating_ray)
                                {
                                    // It is the first time that we have a point
                                    // and
                                    // we have not found a non-saturating ray
                                    // yet.
                                    // - If point `g' saturates `c', then all
                                    // the generators
                                    //   seen so far saturate `c'.
                                    // - If point `g' is included (but does not
                                    // saturate) `c',
                                    //   then all the generators seen so far are
                                    // included in `c'.
                                    // - If point `g' does not satisfy `c', then
                                    // all the
                                    //   generators seen so far are disjoint
                                    // from `c'.
                                    first_point_or_nonsaturating_ray = false;
                                ...
                                else
                                {
                                                ...
                                                Poly_Con_Relation::is_included())))
                                    {
                                        // We have a strict intersection if
                                        // either:
                                        // - `g' satisfies or saturates `c' but
                                        // none of the
                                        //    generators seen so far are
                                        // included in `c'; or
                                        // - `g' does not satisfy `c' and all
                                        // the generators
                                        //    seen so far are included in `c'.
        ...
        case Constraint::STRICT_INEQUALITY:
            {
                // The hyperplane implicitly defined by the strict inequality
                // `c'
                ...
                for (dimension_type i = n_rows; i-- > 0; )
                {
                    ...
                    // Checking whether the generator saturates the strict
                    // inequality.
                ...
                : Poly_Con_Relation::is_disjoint();
                            ...
                            case Generator::CLOSURE_POINT:
                                ...
                                if (first_point_or_nonsaturating_ray)
                                {
                                    ...
                                    // the generators
                                    //   seen so far saturate `c'.
                                    // - If point `g' is included in (but does
                                    // not saturate) `c',
                                    //   then all the generators seen so far are
                                    // included in `c'.
                                    // - If point `g' strictly violates `c',
                                    // then all the
                                    //   generators seen so far are disjoint
                                    // from `c'.
                                    first_point_or_nonsaturating_ray = false;
