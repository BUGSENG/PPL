                      ...
                      Bit_Matrix           & sat)
{
    ...
    for (dimension_type i = 0; i < dest_num_rows; ++i)
    {
        ...
        dest_row_type dest_i(expr, dest_row_type::LINE_OR_EQUALITY,
                             NECESSARILY_CLOSED);
    ...
    else
    {
        for (first_point = num_lines_or_equalities;
             ...
             ++first_point)
        {
            if (dest[first_point].expr.get(Variable(dest.space_dimension())) >
                0)
            {
                break;
            }
...
/*!
   ...
   <CODE>add_and_minimize(bool, Linear_System_Class&, Linear_System_Class&,
      Bit_Matrix&)</CODE>,
                              ...
                              const Source_Linear_System2 & source2)
{
    ...
    while (k1 < old_source1_num_rows && k2 < source2_num_rows)
    {
        ...
        // are sorted and index `k1' only scans the non-pending rows of
        // `source1',
