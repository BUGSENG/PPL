                                      ...
                                      const bool           keep_sorted)
{
    ...
    else
    {
        ...
        const bool last_row_is_pending = (num_rows() - 1 >=
                                          index_first_pending);
                                        ...
                                        dimension_type offset)
{
    ...
    if (first < last)
    {
        bool first_interval_has_pending_rows = (last >
                                                index_first_pending);
        bool second_interval_has_pending_rows = (last + offset >
                                                 index_first_pending);
        bool first_interval_has_not_pending_rows = (first <
                                                    index_first_pending);
        bool second_interval_has_not_pending_rows = (first + offset <
                                                     index_first_pending);
    ...
    {
        // The swaps involved not pending rows, so they may not be sorted
        // anymore.
        set_sorted(false);
    }
...
Linear_System<Row>::remove_rows (const std::vector<dimension_type> & indexes)
{
    ...
    while (itr != itr_end)
    {
        ...
        if (*itr == i)
        {
            // The current row has to be removed, don't increment
            // last_unused_row.
            ++itr;
        }
        else
        {
            // The current row must not be removed, swap it after the last used
            // row.
