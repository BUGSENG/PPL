...
PPL::Congruence::is_tautological () const
{
    if (is_equality())
    {
        return (inhomogeneous_term() == 0) &&
               expr.all_homogeneous_terms_are_zero();
    }

    return (inhomogeneous_term() % modulus() == 0) &&
           expr.all_homogeneous_terms_are_zero();
}
...
PPL_TLS const PPL::Congruence * PPL::Congruence::zero_dim_false_p =
    nullptr;
PPL_TLS const PPL::Congruence * PPL::Congruence::zero_dim_integrality_p =
    nullptr;
