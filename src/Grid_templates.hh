            ...
            if (box.has_lower_bound(v_k, l_n, l_d, closed))
            {
                if (box.has_upper_bound(v_k, u_n, u_d, closed))
                {
                    if (l_n * u_d == u_n * l_d)
                    {
                        ...
                        // by the call to gen_sys.insert() at the end of the
                        // loop.
                        ...
                        // l_d * u_n == abs(l_d * (point_divisor / gcd(l_d,
                        // point_divisor)))
                        ...
                        // point[k + 1] = l_n * point_divisor / gcd(l_d,
                        // point_divisor)
        ...
        for (dimension_type i = space_dim; i-- > 0; )
        {
            if (!visited[i])
            {
                ...
                do
                {
                    ...
                    // The following initialization is only to make the compiler
                    // happy.
                    dimension_type k = 0;
                ...
                // Avoid calling clear_*_minimized() if cycle.size() is less
                // than 2,
                      ...
                      const bool                              generators)
{
         ...
         row_index = pivot_index; row_index-- > 0; )
    {
                ...
                && (sys_dim_kinds[kinds_index] == PARAMETER)))
        {
            ...
            else if ((row_dim_remainder > 0) && (row_dim_remainder >
                                                 pivot_dim_half))
            {
                ++num_rows_to_subtract;
            }

            // Subtract num_rows_to_subtract copies of pivot from row i.  Only
            // the
