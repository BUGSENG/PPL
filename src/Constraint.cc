        ...
        else
        {
            ...
            else
            {
                ...
                if (expr.inhomogeneous_term() <= 0)
                {
                    ...
                    // where k is a non negative integer, cannot be trivially
                    // true.
                    return false;
                }
    ...
    else
    {
        ...
        else
        {
            ...
            if (epsilon_coefficient() >= 0)
            {
                ...
                // If zero, one of the `true' dimensions has a non-zero
                // coefficient.
...
{
    return expr.is_equal_to(y.expr) && kind_ == y.kind_ && topology() ==
           y.topology();
}
...
PPL_TLS const PPL::Constraint * PPL::Constraint::zero_dim_false_p =
    nullptr;
PPL_TLS const PPL::Constraint * PPL::Constraint::zero_dim_positivity_p =
    nullptr;
PPL_TLS const PPL::Constraint * PPL::Constraint::epsilon_geq_zero_p =
    nullptr;
PPL_TLS const PPL::Constraint * PPL::Constraint::epsilon_leq_one_p =
    nullptr;
