        ...
        case PARTIALLY_SATISFIABLE:
            {
                ...
                const dimension_type old_num_cols =
                    initial_context.num_columns();

                if (old_num_cols < new_num_cols)
                {
                    x.initial_context.add_zero_columns(new_num_cols -
                                                       old_num_cols);
                }

                // Computed once for all (to be used inside loop).
                const Variables_Set::const_iterator param_begin =
                    parameters.begin();
                const Variables_Set::const_iterator param_end =
                    parameters.end();
                     ...
                     cs_end = input_cs.end(); cs_i != cs_end; ++cs_i)
                {
                    ...
                    if (!c.expression().all_zeroes_except(parameters, 1,
                                                          c_space_dim + 1))
                    {
                        continue;
                    }
                    ...
                    Row & row = x.initial_context[x.initial_context.num_rows() -
                                                  1];

                    {
                        ...
                        // If the size of param_end is expected to be greater
                        // than the
                        // number of nonzeroes of c in most cases, then this
                        // implementation
                        // can't be optimized further.
                        // itr may still be end(), but it can still be used as
                        // hint.
                    ...
                    if (c.is_equality())
                    {
                        ...
                        last_row =
                            x.initial_context[x.initial_context.num_rows() - 2];
                ...
                x.status = (x.current_solution != nullptr) ? OPTIMIZED :
                           UNSATISFIABLE;
