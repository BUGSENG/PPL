    ...
    gen_sys(ph.space_dimension())
{
    ...
    else
    {
             ...
             gs_end = gs.end(); g != gs_end; ++g)
        {
            ...
            {
                point_expr.linear_combine(g->expr, Coefficient_one(),
                                          Coefficient_one(),
                                          1, space_dim + 1);
        ...
        // If the polyhedron's generator is a (closure) point, the grid line
        // must
         ...
         i_end = gen_sys.end(); i != i_end; ++i)
    {
        ...
        switch (g.type())
        {
            ...
            case Grid_Generator::POINT:
                ...
                else
                {
                    if (point_sp == 0)
                    {
                        ...
                        // Assign `sp' to `point_sp' as `sp' is the scalar
                        // product
                    ...
                    {
                        // A previously considered point p failed to satisfy cg
                        // such that
                        // `point_sp' = `scalar_prod(p, cg)'
                        // so, if we consider the parameter g-p instead of g, we
                        // have
                        // scalar_prod(g-p, cg) = scalar_prod(g, cg) -
                        // scalar_prod(p, cg)
                        //                      = sp - point_sp
                        sp -= point_sp;
            ...
            case Grid_Generator::PARAMETER:
                ...
                if (known_to_intersect)
                {
                    // At least one point satisfies cg.  However, the sum of
                    // such
                    // a point and the parameter g fails to satisfy cg (due to
                    // g).
                    return Poly_Con_Relation::strictly_intersects();
                }
                ...
                if (point_sp != 0)
                {
                    // At least one of any previously encountered points fails
                    // to
                    // satisfy cg.
                    if (point_sp % div == 0)
                    {
            ...
            case Grid_Generator::LINE:
                ...
                if (sp == 0)
                {
                    // Line g satisfies the cg so the relation depends entirely
                    // on
        ...
        for (dimension_type i = gen_sys.num_rows(); i-- > 0; )
        {
            ...
            if (sgn(g_i.coefficient(var)) != 0)
            {
                if (g_i.expression().all_zeroes(1, var.space_dimension())
                    && g_i.expression().all_zeroes(var.space_dimension() + 1,
                                                   space_dim + 1))
                {
...
ok:
    ...
    if (congruences_are_up_to_date())
    {
        ...
        if (congruences_are_minimized())
        {
            ...
            {
#ifndef NDEBUG
                cerr << "Reduced congruences should be lower triangular." <<
                    endl;
#endif
                goto fail;
            }
            ...
            if (dim_kinds.size() != con_sys.space_dimension() +
                1 /* inhomogeneous term */)
            {
#ifndef NDEBUG
...
#ifndef NDEBUG
                cerr << "Kinds in dim_kinds should match those in con_sys." <<
                    endl;
#endif
                goto fail;
            }
...
PPL::Grid::add_recycled_congruences (Congruence_System & cgs)
{
    ...
    {
        throw_dimension_incompatible("add_recycled_congruences(cgs)", "cgs",
                                     cgs);
    }
...
PPL::Grid::add_recycled_grid_generators (Grid_Generator_System & gs)
{
    ...
    {
        throw_dimension_incompatible("add_recycled_grid_generators(gs)", "gs",
                                     gs);
    }
    ...
    if (num_redundant_by_y < x_cs_num_rows)
    {
        ...
        for (dimension_type i = 0, j = 0; i < x_cs_num_rows; ++i)
        {
            if (!redundant_by_y[i])
            {
                ...
                const Grid_Generator_System & y_gs =
                    y.gen_sys;
                     ...
                     y_gs_end = y_gs.end(); k != y_gs_end; ++k)
                {
                    ...
                    {
                        // If `g' is a parameter the congruence modulus must be
                        // scaled
                             ...
                             Coefficient_traits::const_reference modulus)
{
    ...
    if ((var_space_dim <= expr_space_dim) && (var_coefficient != 0))
    {
        ...
        if (modulus < 0)
        {
            generalized_affine_image(var, EQUAL, inverse_expr,
                                     inverse_denominator,
                                     -modulus);
        }
        else
                        ...
                        bool /* wrap_individually */)
{
    ...
    if (cs_p != nullptr)
    {
        ...
        {
            throw_dimension_incompatible("wrap_assign(vs, ...)",
                                         cs_p_space_dim);
        }
    }
    ...
    if ((o == OVERFLOW_IMPOSSIBLE) || (o == OVERFLOW_WRAPS))
    {
             ...
             vars_end = vars.end(); i != vars_end; ++i)
        {
            ...
            if (f_n == 0)
            {
                ...
                {
                    // The value is outside the range of the bounded integer
                    // type.
                    ...
                    // The value v_n for `x' is wrapped modulo the
                    // 'wrap_frequency'.
                    v_n %= wrap_frequency;
            ...
            else
            {
                ...
                PPL_ASSERT(o == OVERFLOW_IMPOSSIBLE && 2 * f_n <
                           wrap_frequency);
            }
        }
