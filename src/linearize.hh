...
sub_linearize (const
               ...
               const FP_Oracle<Target,
                               FP_Interval_Type>              & oracle,
               const std::map<dimension_type,
                              Linear_Form<FP_Interval_Type> > & lf_store,
               Linear_Form<FP_Interval_Type>                                  &
               result)
{
               ...
               result)
{
    ...
    // true if we intervalize the first form, false if we intervalize the
    // second.
...
template <typename Target, typename FP_Interval_Type>
static bool
cast_linearize (const
                Cast_Operator<Target>                          & cast_expr,
                const FP_Oracle<Target,
                                FP_Interval_Type>              & oracle,
                const std::map<dimension_type,
                               Linear_Form<FP_Interval_Type> > & lf_store,
                ...
                result)
{
    ...
    else
    {
        ...
        if (is_less_precise_than(
                Float<analyzer_format>::Binary::floating_point_format,
                analyzed_format)
            || (result == FP_Linear_Form(FP_Interval_Type(0)))
            || (result == FP_Linear_Form(FP_Interval_Type(1))))
        {
...
bool
linearize (const
           Concrete_Expression<Target>                    & expr,
           const FP_Oracle<Target,
                           FP_Interval_Type>              & oracle,
           const std::map<dimension_type,
                          Linear_Form<FP_Interval_Type> > & lf_store,
        ...
        case Unary_Operator<Target>::KIND:
            {
                ...
                switch (uop_expr->unary_operator())
                {
                    case Unary_Operator<Target>::UPLUS:
                        return linearize(*(uop_expr->argument()), oracle,
                                         lf_store, result);

                    case Unary_Operator<Target>::UMINUS:

                        if (!linearize(*(uop_expr->argument()), oracle,
                                       lf_store, result))
                        {
                            return false;
                        }
        ...
        case Binary_Operator<Target>::KIND:
            {
                ...
                switch (bop_expr->binary_operator())
                {
                    case Binary_Operator<Target>::ADD:
                        return add_linearize(*bop_expr, oracle, lf_store,
                                             result);

                    case Binary_Operator<Target>::SUB:
                        return sub_linearize(*bop_expr, oracle, lf_store,
                                             result);

                    case Binary_Operator<Target>::MUL:
                        return mul_linearize(*bop_expr, oracle, lf_store,
                                             result);

                    case Binary_Operator<Target>::DIV:
                        return div_linearize(*bop_expr, oracle, lf_store,
                                             result);
        ...
        case Approximable_Reference<Target>::KIND:
            {
                ...
                if (!oracle.get_associated_dimensions(*ref_expr,
