...
PPL::Dense_Row::resize (const dimension_type new_size)
{
    ...
    else
    {
        if (new_size > capacity())
        {
            ...
            Coefficient * const  new_vec      = impl.coeff_allocator.allocate(
                new_capacity);
                                      ...
                                      const dimension_type i)
{
    ...
    {
        Dense_Row            new_row;
        const dimension_type new_capacity = compute_capacity(new_size,
                                                             max_size());

        // This may throw.
        new_row.impl.vec = new_row.impl.coeff_allocator.allocate(
            new_capacity);
...
#    pragma GCC diagnostic pop
        ...
        {
            // impl.vec[impl.size]..impl.vec[target_size-1] are still
            // unconstructed,
