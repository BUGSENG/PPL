                         ...
                         const dimension_type end)
{
    ...
    row_e.linear_combine(old_pivot_e, reduced_pivot_col, -reduced_row_col,
                         start, end);
...
Grid::simplify (Grid_Generator_System & ggs, Dimension_Kinds & dim_kinds)
{
    ...
    for (dimension_type dim = 0; dim < num_columns; ++dim)
    {
        ...
        else
        {
            ...
            while (row_index < num_rows - 1)
            {
                ...
                if (row.is_line())
                {
                    ...
                    else
                    {
                        ...
                        swap(row, pivot);
                        pivot_is_line = true;
                        reduce_parameter_with_line(row, pivot, dim,
                                                   ggs.sys.rows,
                                                   num_columns + 1);
                    }
                }
            ...
            reduce_reduced<Grid_Generator_System>(ggs.sys.rows, dim,
                                                  pivot_index, dim,
                                                  num_columns - 1, dim_kinds);
    ...
    const Coefficient & system_divisor =
        ggs.sys.rows[0].expr.inhomogeneous_term();
...
Grid::simplify (Congruence_System & cgs, Dimension_Kinds & dim_kinds)
{
    ...
    for (dimension_type dim = num_columns; dim-- > 0; )
    {
        ...
        else
        {
            ...
            while (row_index < num_rows - 1)
            {
                ...
                if (row.is_equality())
                {
                    ...
                    else
                    {
                        ...
                        reduce_congruence_with_equality(row, pivot, dim,
                                                        cgs.rows);
                    }
                }
                else
            ...
            reduce_reduced<Congruence_System>(cgs.rows, dim, pivot_index, 0,
                                              dim, dim_kinds, false);
    ...
    if (pivot_index > 0)
    {
...
#ifndef NDEBUG
        {
            const bool ret = rows_are_zero<Congruence_System, Congruence>(cgs,
                                                                          // index
                                                                          // of
                                                                          // first
                                                                          pivot_index,
        ...
        switch (dim_kinds[0])
        {
            ...
            case EQUALITY:
                // The last row is a false equality, as all the coefficient
                // terms
    ...
    else
    {
        ...
        if (num_rows > 0)
        {
            ...
            PPL_ASSERT(ret == true);
#endif
            // Ensure that a single row will remain for the integrality
            // congruence.
    ...
    reduce_reduced<Congruence_System>(cgs.rows, 0, cgs.num_rows() - 1, 0, 0,
                                      dim_kinds, false);
