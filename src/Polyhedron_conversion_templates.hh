...
#    endif // PPL_QUICK_ADJ_TEST
        ...
        if (index_non_zero < num_lines_or_equalities)
        {
            ...
            if (scalar_prod[index_non_zero] < 0)
            {
                // The ray `dest_rows[index_non_zero]' lies on the wrong
                // half-space:
                ...
                // The modified row may still not be OK(), so don't assert OK
                // here.
                // They are all checked at the end of this function.
            }

            // Having changed a line to a ray, we set `dest_rows' to be a
            // non-sorted system, we decrement the number of lines of
            // `dest_rows'
            // and, if necessary, we move the new ray below all the remaining
            // lines.
            dest_sorted = false;
                 ...
                 i = index_non_zero; i < num_lines_or_equalities; ++i)
            {
                if (scalar_prod[i] != 0)
                {
                    ...
                    // The modified row may still not be OK(), so don't assert
                    // OK here.
                    // They are all checked at the end of this function.
                    scalar_prod[i] = 0;
        ...
        while (inf_bound > sup_bound)
        {
            ...
            if (sp_sign == 0)
            {
                ...
                swap(dest.sys.rows[sup_bound],
                     dest.sys.rows[lines_or_equal_bound]);
                swap(scalar_prod[sup_bound], scalar_prod[lines_or_equal_bound]);
                swap(sat[sup_bound], sat[lines_or_equal_bound]);
#if PPL_QUICK_ADJ_TEST
                swap(sat_num_ones[sup_bound],
                     sat_num_ones[lines_or_equal_bound]);
#endif // PPL_QUICK_ADJ_TEST
                ++lines_or_equal_bound;
                ++sup_bound;
                dest_sorted = false;
        ...
        if (sup_bound == dest_num_rows)
        {
            ...
            else
            {
                ...
                {
                    recyclable_dest_rows.resize(recyclable_dest_rows.size() +
                                                1);
                    swap(dest.sys.rows.back(), recyclable_dest_rows.back());
                    dest.sys.rows.pop_back();
                    --dest_num_rows;
        ...
        for (dimension_type i = lines_or_equal_bound; i < sup_bound; ++i)
        {
            for (dimension_type j = sup_bound; j < bound; ++j)
            {
                ...
                // are adjacent. Equivalently, adjacency holds if
                // `new_satrow_ones'
                ...
                // Perform the full (combinatorial) adjacency test.
                {
                    ...
                    for (dimension_type l = num_lines_or_equalities; l < bound;
                         ++l)
                    {
                        if ((l != i) && (l != j)
                            && subset_or_equal(sat[l], new_satrow))
                        {
                            ...
                            // constraints
                            // saturated by both `dest_rows[i]' and
                            // `dest_rows[j]'.
                            redundant = true;
                            break;
                        }
