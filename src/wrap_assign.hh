                 ...
                 Coefficient                       & tmp2)
{
    ...
    for (Wrap_Translations::const_iterator i = first; i != end; ++i)
    {
        ...
        const Coefficient           & first_quadrant =
            wrap_dim_translations.first_quadrant;
        const Coefficient & last_quadrant =
            wrap_dim_translations.last_quadrant;
    ...
    else
    {
        ...
        for (quadrant = first_quadrant; quadrant <= last_quadrant; ++quadrant)
        {
            if (quadrant != 0)
            {
                ...
                p.affine_image(x, x - shift, 1);
                wrap_assign_col(dest, p, vars, first + 1, end, w, min_value,
                                max_value,
                                cs_p, tmp);
            }
            else
            {
                wrap_assign_col(dest, src, vars, first + 1, end, w, min_value,
                                max_value,
                                cs_p, tmp);
            }
        }
