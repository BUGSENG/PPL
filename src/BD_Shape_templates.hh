        ...
        switch (g.type())
        {
            ...
            case Generator::CLOSURE_POINT:
                ...
                if (!dbm_initialized)
                {
                    ...
                    {
                        const Coefficient & g_i = g.expression().get(Variable(
                                                                         i -
                                                                         1));
                        ...
                        for (dimension_type j = space_dim; j > 0; --j)
                        {
                            if (i != j)
                            {
                                const Coefficient & g_j = g.expression().get(
                                    Variable(j - 1));
                    ...
                    for (dimension_type j = space_dim; j > 0; --j)
                    {
                        const Coefficient & g_j = g.expression().get(Variable(
                                                                         j -
                                                                         1));
                    ...
                    // Note: no need to initialize the first element of the main
                    // diagonal.
                }
                else
                {
         ...
         gs_i != gs_end; ++gs_i)
    {
        ...
        switch (g.type())
        {
            case Generator::LINE:
                ...
                for (dimension_type i = space_dim; i > 0; --i)
                {
                    const Coefficient & g_i = g.expression().get(Variable(i -
                                                                          1));
                    DB_Row<N> & dbm_i = dbm[i];
                ...
                for (Generator::expr_type::const_iterator i =
                         g.expression().begin(),
    ...
    for (dimension_type i = 1; i <= space_dim; ++i)
    {
        ...
        if (i != leader)
        {
            ...
            else
            {
                ...
                cgs.insert(denom * Variable(leader - 1) - denom * Variable(i -
                                                                           1) ==
                           numer);
            }
        }
        ...
        else
        {
                 ...
                 j_end = le.lower_bound(Variable(i - 1)); j != j_end; ++j)
            {
                ...
                if (is_additive_inverse(dbm[j_dim][i], tmp))
                {
                    ...
                    numer_denom(tmp, numer, denom);
                    // Modifying le invalidates the iterators, but it's not a
                    // problem
    ...
    for (dimension_type i = predecessor_size; i-- > 1; )
    {
        if (i == predecessor[i])
        {
            ...
            for (dimension_type j = i; j-- > 0; )
            {
                ...
                {
                    // Choose as predecessor the variable having the smaller
                    // index.
                    predecessor[i] = j;
                    break;
                }
            ...
            for (dimension_type j = 0; j <= space_dim; ++j)
            {
                ...
                if (j != leader_j)
                {
                    if (!redundancy_dbm[i][j])
                    {
                        if (t == 1)
                        {
                            // Two non-leaders cannot be connected with the same
                            // leader.
                            return false;
                        }
                        else
                        {
                            if (leader_j != i)
                            {
                                // The variables are not in the same equivalence
                                // class.
                                return false;
                            }
                            else
            ...
            for (dimension_type j = 0; j <= space_dim; ++j)
            {
                if (!redundancy_dbm[i][j])
                {
                    ...
                    else
                    {
                        if (t == 1)
                        {
                            // The variables cannot be connected with the same
                            // leader.
                            return false;
                        }
                        else
      ...
      ? Coefficient_zero() : g.coefficient(Variable(i - 1));
        ...
        ? Coefficient_zero() : g.coefficient(Variable(j - 1));
            ...
            else
            {
                ...
                {
                    // We have the binary inequality constraint: denom*y -
                    // denom*x <= b.
                    // Compute the scalar product.
        ...
        if ((i != predecessor[i]) && !dealt_with[i])
        {
            ...
            while (true)
            {
                ...
                if (j == predecessor_j)
                {
                    ...
                    // Here we dealt with `j' (i.e., `predecessor_j'), but it is
                    // useless
    ...
    for (dimension_type i = x_space_dim + 1; i-- > 0; )
    {
        ...
        for (dimension_type j = x_space_dim + 1; j-- > 0; )
        {
            ...
            if (!x_red_i[j])
            {
                ...
                // Build skeleton DB constraint (having the right space
                // dimension).
                db_expr = zero_expr;
            ...
            if (!y_red_i[j])
            {
                ...
                {
                    // Check if same constraint was added when considering
                    // x_dbm_ij.
                ...
                for (dimension_type k = x_space_dim + 1; k-- > 0; )
                {
                    ...
                    for (dimension_type ell = x_space_dim + 1; ell-- > 0; )
                    {
                        ...
                        if (y_k_ell < x_k[ell])
                        {
                            ...
                            if (integer_upper_bound)
                            {
                                // Note: adding 1 rather than 2 (as in Theorem
                                // 5.3)
                                // so as to later test for < rather than <=.
                                add_assign_r(lhs, lhs, temp_one,
                                             ROUND_NOT_NEEDED);
                            }

                            // Testing for < in both the rational and integer
                            // case.
                            if (lhs < rhs)
                            {
                                return false;
    ...
    if (x.marked_empty())
    {
        ...
        for (i = 1; i <= dim; ++i)
        {
            ...
            for (j = 1; j <= dim; ++j)
            {
                ...
                {
                    // FIXME: if N is a float or bounded integer type, then
                    // we also need to check that we are actually able to
                    // construct
                    // a constraint inconsistent with respect to this one.
                    goto found;
                }
...
BD_Shape<T>::add_space_dimensions_and_embed (const dimension_type m)
{
    ...
    const bool           was_zero_dim_univ = (!marked_empty() && space_dim ==
                                              0);
                                 ...
                                 BD_Shape                & limiting_shape) const
{
         ...
         cs_end = cs.end(); cs_i != cs_end; ++cs_i)
    {
        ...
        // Constraints that are not bounded differences are ignored.
        if (BD_Shape_Helpers::extract_bounded_difference(c, num_vars, i, j,
                                                         coeff))
        {
            // Select the cell to be modified for the "<=" part of the
            // constraint,
            // and set `coeff' to the absolute value of itself.
            const bool     negative = (coeff < 0);
            const N      & x        = negative ? dbm[i][j] : dbm[j][i];
            ...
            if (x <= d)
            {
                ...
                {
                    // Compute the bound for `y', rounding towards plus
                    // infinity.
...
BD_Shape<T>::BHMZ05_widening_assign (const BD_Shape & y, unsigned * tp)
{
    ...
    PPL_ASSERT(marked_shortest_path_closed() &&
               y.marked_shortest_path_closed());
    // Minimize `y'.
    y.shortest_path_reduction_assign();
    ...
    switch (relsym)
    {
        case EQUAL:
            {
                ...
                {
                    const dimension_type i_dim =
                        i.variable().space_dimension();
                    ...
                    if (sign_i > 0)
                    {
                        ...
                        if (pinf_count <= 1)
                        {
                            ...
                            {
                                add_mul_assign_r(sum, coeff_i, approx_i,
                                                 ROUND_UP);
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        // Note: using temporary named `coeff_i' to store
                        // -coeff_i.
                        ...
                        if (pinf_count <= 1)
                        {
                            ...
                            if (!is_plus_infinity(approx_minus_i))
                            {
                                add_mul_assign_r(sum, coeff_i, approx_minus_i,
                                                 ROUND_UP);
                            }
                            else
                            {
                        ...
                        if (neg_pinf_count <= 1)
                        {
                            ...
                            if (!is_plus_infinity(approx_i))
                            {
                                add_mul_assign_r(neg_sum, coeff_i, approx_i,
                                                 ROUND_UP);
                            }
                            else
                            {
                ...
                // In the following, shortest-path closure will be definitely
                // lost.
                reset_shortest_path_closed();
                ...
                if (pinf_count <= 1)
                {
                    ...
                    else
                    {
                        // Here `pinf_count == 1'.
                        if ((pinf_index != v)
                            && (sc_expr.get(Variable(pinf_index - 1)) ==
                                sc_denom))
                        {
                ...
                if (neg_pinf_count <= 1)
                {
                    ...
                    if (neg_pinf_count == 0)
                    {
                        ...
                        dbm_v[0] = neg_sum;
                        // Deduce constraints of the form `u - v', where `u !=
                        // v'.
                        deduce_u_minus_v_bounds(v, w, sc_expr, sc_denom,
                                                neg_sum);
                    }
                    // Here `neg_pinf_count == 1'.
                    else if ((neg_pinf_index != v)
                             && (sc_expr.get(Variable(neg_pinf_index - 1)) ==
                                 sc_denom))
                    {
        ...
        case LESS_OR_EQUAL:
                 ...
                 i_end = sc_expr.lower_bound(Variable(w)); i != i_end; ++i)
            {
                ...
                const N & approx_i = (sign_i > 0) ? dbm_0[i_dim] :
                                     dbm[i_dim][0];
            ...
            if (sc_denom != 1)
            {
                ...
                // achieved
                // by rounding upwards `minus_sc - denom' and negating again the
                // result.
                PPL_DIRTY_TEMP(N, down_sc_denom);
                assign_r(down_sc_denom, minus_sc_denom, ROUND_UP);
                neg_assign_r(down_sc_denom, down_sc_denom, ROUND_UP);
        ...
        case GREATER_OR_EQUAL:
                 ...
                 i_end = sc_expr.lower_bound(Variable(w)); i != i_end; ++i)
            {
                ...
                // Choose carefully: we are approximating `-sc_expr'.
                const N & approx_i = (sign_i > 0) ? dbm[i_dim][0] :
                                     dbm_0[i_dim];
            ...
            if (sc_denom != 1)
            {
                ...
                // approximated towards zero. Since `sc_denom' is known to be
                // positive,
                // this amounts to rounding downwards, which is achieved by
                // rounding
            ...
            if (w == v)
            {
                ...
                if (a == denominator)
                {
                    ...
                    else
                    {
                        ...
                        // Both shortest-path closure and reduction are
                        // preserved.
                    }
                }
                else
         ...
         i_end = sc_expr.lower_bound(Variable(w)); i != i_end; ++i)
    {
        ...
        if (sign_i > 0)
        {
            ...
            if (neg_pinf_count <= 1)
            {
                ...
                {
                    add_mul_assign_r(neg_sum, coeff_i, up_approx_minus_i,
                                     ROUND_UP);
                }
                else
                {
        ...
        else
        {
            ...
            if (pos_pinf_count <= 1)
            {
                ...
                if (!is_plus_infinity(up_approx_minus_i))
                {
                    add_mul_assign_r(pos_sum, coeff_i, up_approx_minus_i,
                                     ROUND_UP);
                }
                else
                {
...
void
BD_Shape<T>::affine_form_image (const Variable
                                var,
...
template <typename Interval_Info>
void BD_Shape<T>
::two_variables_affine_form_image (const
                                   dimension_type
                                   & var_id,
                                   const Linear_Form< Interval<T,
                                                               Interval_Info> >
    ...
    const Linear_Form< Interval<T, Interval_Info> > & right)
{
    ...
    else if (left_t == 1)
    {
        ...
        {
            if ((right_t == 0) || ((right_w_coeff == 1) || (right_w_coeff ==
                                                            -1)))
            {
                left_one_var_refine(left_w_id, right_t, right_w_id, left,
                                    right);
                PPL_ASSERT(OK());
                return;
            }
...
template <typename Interval_Info>
void
BD_Shape<T>::left_inhomogeneous_refine (const
                                        dimension_type & right_t,
                                        const dimension_type
                                        &
                                        right_w_id,
                                        const Linear_Form< Interval<T,
...
void
BD_Shape<T>
::left_one_var_refine (const
                       dimension_type       & left_w_id,
    ...
    else if (right_t == 1)
    {
        ...
        if (is_left_coeff_one && is_right_coeff_one)
        {
            ...
            sub_assign_r(c_plus_minus_a_minus, right_c.upper(), left_a.lower(),
                         ROUND_UP);
            add_dbm_constraint(right_w_id + 1, left_w_id + 1,
        ...
        if (is_left_coeff_minus_one && is_right_coeff_minus_one)
        {
            ...
            sub_assign_r(c_plus_minus_a_minus, right_c.upper(), left_a.lower(),
                         ROUND_UP);
            add_dbm_constraint(left_w_id + 1, right_w_id + 1,
                               c_plus_minus_a_minus);
            return;
        }
    }
        ...
        if ((a == denominator) || (a == minus_denom))
        {
            ...
            switch (relsym)
            {
                case LESS_OR_EQUAL:
                    ...
                    {
                        // `expr' is of the form: a*v + b.
                        // Shortest-path closure and reduction are not
                        // preserved.
                        reset_shortest_path_closed();
                        ...
                        if (a == denominator)
                        {
                            ...
                            // into the constraint `v - w <= dbm_wv +
                            // b/denominator';
                            ...
                            for (dimension_type i = space_dim + 1; i-- > 0; )
                            {
                                ...
                                add_assign_r(dbm_iv, dbm_iv, d, ROUND_UP);
                                assign_r(dbm_v[i], PLUS_INFINITY,
                                         ROUND_NOT_NEEDED);
                            }
                        }
                        else
                    ...
                    else
                    {
                        ...
                        // Shortest-path closure is preserved, but not
                        // reduction.
                        if (marked_shortest_path_reduced())
                        {
                            reset_shortest_path_reduced();
                        ...
                        {
                            // Here a == -denominator, so that we should be
                            // adding
                            // the constraint `v <= b/denominator - w'.
                            // Approximate it by computing a lower bound for
                            // `w'.
                            const N & dbm_w0 = dbm[w][0];

                            if (!is_plus_infinity(dbm_w0))
                            {
                                // Add the constraint `v <= b/denominator -
                                // lb_w'.
                ...
                case GREATER_OR_EQUAL:
                    ...
                    if (w == v)
                    {
                        ...
                        if (a == denominator)
                        {
                            ...
                            // into the constraint `w - v <= dbm_vw -
                            // b/denominator';
                            ...
                            for (dimension_type i = space_dim + 1; i-- > 0; )
                            {
                                ...
                                assign_r(dbm[i][v], PLUS_INFINITY,
                                         ROUND_NOT_NEEDED);
                            }
                        }
                        else
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // Approximate it by computing an upper bound for
                            // `w'.
                            const N & dbm_0w = dbm_0[w];

                            if (!is_plus_infinity(dbm_0w))
                            {
                                // Add the constraint `-v <= ub_w -
                                // b/denominator'.
    ...
    for (dimension_type i = 1; i <= space_dim; ++i)
    {
        ...
        if (i != leader)
        {
            ...
            else
            {
                ...
                PPL_ASSERT(!is_plus_infinity(dbm[i][leader]));
                numer_denom(dbm[i][leader], numer, denom);
                cs.insert(denom * Variable(leader - 1) - denom * Variable(i -
                                                                          1) ==
                          numer);
            }
        }
    ...
    for (dimension_type l_i = 1; l_i < num_leaders; ++l_i)
    {
        ...
        for (dimension_type l_j = l_i + 1; l_j < num_leaders; ++l_j)
        {
            ...
            if (!red_i[j])
            {
                ...
                cs.insert(denom * Variable(j - 1) - denom * Variable(i - 1) <=
                          numer);
            }
            ...
            if (!redundancy_dbm[j][i])
            {
                ...
                cs.insert(denom * Variable(i - 1) - denom * Variable(j - 1) <=
                          numer);
            }
        }
    }
            ...
            for (dimension_type i = 0; i <= n; ++i)
            {
                for (dimension_type j = i + 1; j <= n; ++j)
                {
                    ...
                    if (is_additive_inverse(c_j_i, c_i_j))
                    {
                        ...
                        if (i == 0)
                        {
                            // We have got a equality constraint with one
                            // variable.
                            s << Variable(j - 1);
                            s << " = " << c_i_j;
                        }
                        ...
                        {
                            // We have got a equality constraint with two
                            // variables.
                            if (sgn(c_i_j) >= 0)
                            {
                                s << Variable(j - 1);
                    ...
                    else
                    {
                        ...
                        if (!is_plus_infinity(c_j_i))
                        {
                            ...
                            if (i == 0)
                            {
                                // We have got a constraint with only one
                                // variable.
                                s << Variable(j - 1);
...
template<typename Interval_Info>
void
BD_Shape<T>::throw_dimension_incompatible (const
                                           char * method,
                                           const char
                                           * lf_name,
                                           const Linear_Form< Interval<T,
