    ...
    status()
{
         ...
         gs_i = gs_begin; gs_i != gs_end; ++gs_i)
    {
        ...
        if (g.is_point())
        {
            ...
            if (point_seen)
            {
                // This is not the first point: `seq' already contains valid
                // values.
                // TODO: If the variables in the expression that have
                // coefficient 0
                // have no effect on seq[i], this loop can be optimized using
                // Generator::expr_type::const_iterator.
                for (dimension_type i = space_dim; i-- > 0; )
                {
                    assign_r(q.get_num(), g.coefficient(Variable(i)),
                             ROUND_NOT_NEEDED);
            ...
            case Generator::CLOSURE_POINT:
                {
                    ...
                    // TODO: If the variables in the expression that have
                    // coefficient 0
                    // have no effect on seq[i], this loop can be optimized
                    // using
                    // Generator::expr_type::const_iterator.
                    for (dimension_type i = space_dim; i-- > 0; )
                    {
        ...
        else
        {
            ...
            switch (sgn(bound_diff))
            {
                ...
                case -1:
                    ...
                    {
                        assign_r(bound_diff, i.upper(), ROUND_NOT_NEEDED);
                        sub_assign_r(bound_diff, bound_diff, bound,
                                     ROUND_NOT_NEEDED);
...
Box<ITV>::relation_with (const Constraint & c) const
{
    ...
    else
    {
        ...
        for (Constraint::expr_type::const_iterator i = e.begin(), i_end =
                 e.end();
             i != i_end; ++i)
        {
            ...
            // FIXME: an add_mul_assign() method would come handy here.
            t.build(seq[v.id()].lower_constraint(),
                    seq[v.id()].upper_constraint());
            t *= m;
            r += t;
        }
...
Box<ITV>::relation_with (const Generator & g) const
{
    ...
    if (g.is_line_or_ray())
    {
        if (g.is_line())
        {
            ...
            for (Generator::expr_type::const_iterator i = e.begin(), i_end =
                     e.end();
                 i != i_end; ++i)
            {
    ...
    for (dimension_type i = g_space_dim; i-- > 0; )
    {
        ...
        assign_r(g_coord.get_num(), g.coefficient(Variable(i)),
                 ROUND_NOT_NEEDED);
                ...
                if (!seq_i.lower_is_boundary_infinity())
                {
                    if (seq_i.lower_is_open())
                    {
                        if (!seq_i.upper_is_boundary_infinity())
                        {
                            if (seq_i.upper_is_open())
                            {
                                // Bounded and open interval: compute middle
                                // point.
                                assign_r(g_coord, seq_i.lower(),
                                         ROUND_NOT_NEEDED);
                                PPL_DIRTY_TEMP(mpq_class, q_seq_i_upper);
                                assign_r(q_seq_i_upper, seq_i.upper(),
                                         ROUND_NOT_NEEDED);
                       ...
                       bool                                wrap_individually)
{
    ...
    {
        I_Constraint<Coefficient> lower = i_constraint(GREATER_OR_EQUAL,
                                                       min_value);
        I_Constraint<Coefficient> upper = i_constraint(LESS_OR_EQUAL,
                                                       max_value);
    ...
    if (cs_p == nullptr)
    {
        ...
        switch (o)
        {
            case OVERFLOW_WRAPS:
                ...
                for (Variables_Set::const_iterator i = vars.begin(); i !=
                     vs_end; ++i)
                {
...
Box<ITV>::simplify_using_context_assign (const Box & y)
{
    ...
    if (x.is_empty())
    {
        ...
        for (dimension_type i = 0; i < num_dims; ++i)
        {
            ...
            else
            {
                ...
                if (seq_i.is_empty())
                {
                    // We were not able to assign to `seq_i' a non-empty
                    // interval:
                    // reset `seq_i' to the universe interval and keep
                    // searching.
                    seq_i.assign(UNIVERSE);
                    continue;
                }
    ...
    for (dimension_type i = 0; i < num_dims; ++i)
    {
        if (!x.seq[i].simplify_using_context_assign(y.seq[i]))
        {
            ...
            // The intersection of `x' and `y' is empty due to the i-th
            // interval:
...
maybe_refine_upper_1:
            ...
            for (Constraint::expr_type::const_iterator i = c_e.begin(),
                 i_end = c_e.lower_bound(Variable(c_space_dim)); i != i_end;
                 ++i)
            {
                const Variable i_var = i.variable();
         ...
         i_end = c_e.lower_bound(Variable(c_space_dim)); i != i_end; ++i)
    {
        ...
        switch (c.type())
        {
            ...
            case Constraint::NONSTRICT_INEQUALITY:
                rel = (sgn_coefficient_i > 0) ? GREATER_OR_EQUAL :
                      LESS_OR_EQUAL;
            break;
    ...
    else
    {
        ...
        if (maximize(ub_expr, max_numer, max_denom, max_included))
        {
            if (minimize(lb_expr, min_numer, min_denom, min_included))
            {
                ...
                if (denominator > 0)
                {
                    Relation_Symbol gr = min_included ? GREATER_OR_EQUAL :
                                         GREATER_THAN;
                    Relation_Symbol lr = max_included ? LESS_OR_EQUAL :
                                         LESS_THAN;
                ...
                {
                    Relation_Symbol gr = max_included ? GREATER_OR_EQUAL :
                                         GREATER_THAN;
                    Relation_Symbol lr = min_included ? LESS_OR_EQUAL :
                                         LESS_THAN;
                               ...
                               Coefficient_traits::const_reference denominator)
{
    ...
    bool max_included;
    bool bound_above = maximize(denominator * var, max_numer, max_denom,
                                max_included);
    ...
    bool min_included;
    bool bound_below = minimize(denominator * var, min_numer, min_denom,
                                min_included);
        ...
        if (coeff > 0)
        {
            ...
            switch (relsym)
            {
                case LESS_OR_EQUAL:
                    ...
                    if (max_rhs)
                    {
                        Relation_Symbol rel = max_included ? LESS_OR_EQUAL :
                                              LESS_THAN;
                ...
                case EQUAL:
                    {
                        ...
                        if (max_rhs)
                        {
                            u.set(max_included ? LESS_OR_EQUAL : LESS_THAN,
                                  q_max);
                        }

                        if (min_rhs)
                        {
                            l.set(min_included ? GREATER_OR_EQUAL :
                                  GREATER_THAN, q_min);
                        }
                ...
                case GREATER_OR_EQUAL:
                    ...
                    if (min_rhs)
                    {
                        Relation_Symbol rel = min_included ? GREATER_OR_EQUAL :
                                              GREATER_THAN;
        ...
        else
        {
            ...
            switch (relsym)
            {
                case GREATER_OR_EQUAL:
                    ...
                    if (min_rhs)
                    {
                        Relation_Symbol rel = min_included ? LESS_OR_EQUAL :
                                              LESS_THAN;
                ...
                case EQUAL:
                    {
                        ...
                        if (max_rhs)
                        {
                            l.set(max_included ? GREATER_OR_EQUAL :
                                  GREATER_THAN, q_max);
                        }

                        if (min_rhs)
                        {
                            u.set(min_included ? LESS_OR_EQUAL : LESS_THAN,
                                  q_min);
                        }
                ...
                case LESS_OR_EQUAL:
                    ...
                    if (max_rhs)
                    {
                        Relation_Symbol rel = max_included ? GREATER_OR_EQUAL :
                                              GREATER_THAN;
                            ...
                            Box                     & limiting_box) const
{
         ...
         cs_end = cs.end(); cs_i != cs_end; ++cs_i)
    {
        ...
        if (!Box_Helpers::extract_interval_constraint(c, c_num_vars,
                                                      c_only_var))
        {
            continue;
        }
        ...
        if (c_num_vars != 0)
        {
            ...
            if (interval_relation(seq[c_only_var], c.type(), n, d)
                == Poly_Con_Relation::is_included())
            {
                limiting_box.add_interval_constraint_no_check(c_only_var,
                                                              c.type(),
                                                              n, d);
            }
        }
