...
PPL_TLS const PPL::Generator * PPL::Generator::zero_dim_point_p =
    nullptr;
PPL_TLS const PPL::Generator * PPL::Generator::zero_dim_closure_point_p =
    nullptr;
...
PPL::Generator::is_matching_closure_point (const Generator & p) const
{
    ...
    else
    {
        ...
        gcd_assign(gcd, cp.expr.inhomogeneous_term(),
                   p.expr.inhomogeneous_term());
        ...
        const Coefficient & cp_div = rel_prime ? cp.expr.inhomogeneous_term() :
                                     cp_0_scaled;
        const Coefficient & p_div = rel_prime ? p.expr.inhomogeneous_term() :
                                    p_0_scaled;

        return cp.expr.is_equal_to(p.expr, p_div, cp_div, 1,
                                   cp.expr.space_dimension());
    }
}
