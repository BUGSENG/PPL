...
Interval<To_Boundary, To_Info>::difference_assign (const From & x)
{
    ...
    if (nl)
    {
        ...
        else
        {
            ...
            rl = complement(LOWER, lower(), info(), UPPER, f_upper(x), f_info(
                                x));
        }
    }
    else if (nu)
                                                   ...
                                                   const From2 & y)
{
    ...
    if (nl)
    {
        ...
        {
            rl = complement(LOWER, lower(), info(), UPPER, f_upper(y), f_info(
                                y));
            ru = Boundary_NS::assign(UPPER, upper(), info(), UPPER, f_upper(x),
                                     f_info(x));
        }
    }
    else if (nu)
...
Interval<To_Boundary, To_Info>::neg_assign (const From & x)
{
    ...
    rl = Boundary_NS::neg_assign(LOWER, to_lower, to_info, UPPER, f_upper(x),
                                 f_info(x));
    ru = Boundary_NS::neg_assign(UPPER, upper(), to_info, LOWER, f_lower(x),
                                 f_info(x));
