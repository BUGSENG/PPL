...
#define PPL_DECLARE_FUN1_0_1(name, ret_type, qual, type, after1)             \
    ...
    struct PPL_FUNCTION_CLASS(name);                                         \
    template <typename Policy, typename type>                                \
    inline ret_type PPL_U(name)(PPL_U(qual) PPL_U(type) & arg, PPL_U(        \
                                    after1) a1) {                            \
        return                                                               \
...
#define PPL_DECLARE_FUN1_1_1(name, ret_type, before1, qual, type, after1)        \
    ...
    struct PPL_FUNCTION_CLASS(name);                                             \
    template <typename Policy, typename type>                                    \
    inline ret_type PPL_U(name)(PPL_U(before1) b1, PPL_U(qual) PPL_U(type) &     \
                                arg,                                             \
                                PPL_U(after1) a1) {                              \
        return                                                                   \
...
#define PPL_SPECIALIZE_CMP(func, Type1, Type2)                               \
    PPL_SPECIALIZE_FUN2_0_0(cmp, func, Result_Relation, const, Type1, const, \
                            Type2)
#define PPL_SPECIALIZE_CLASSIFY(func, Type)                                  \
    PPL_SPECIALIZE_FUN1_0_3(classify, func, Result, const, Type, bool, bool, \
                            bool)
