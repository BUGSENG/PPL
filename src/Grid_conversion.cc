                  ...
                  Dimension_Kinds & dim_kinds)
{
    ...
    for (dimension_type dim = dims; dim-- > 0; )
    {
        ...
        else
        {
            ...
            if (dim_kinds[dim] == PARAMETER)
            {
                ...
                // of that row.
                lcm_assign(diagonal_lcm, diagonal_lcm,
    ...
    for (dimension_type dim = dims, i = 0; dim-- > 0; )
    {
        if (dim_kinds[dim] != CON_VIRTUAL)
        {
            ...
            reduce_reduced<Congruence_System>(dest.rows, dim, i++, 0, dim,
                                              dim_kinds, false);
        }
    }
                  ...
                  Dimension_Kinds & dim_kinds)
{
    ...
    for (dimension_type dim = dims; dim-- > 0; )
    {
        ...
        else
        {
            if (dim_kinds[dim] == PROPER_CONGRUENCE)
            {
                ...
                lcm_assign(diagonal_lcm, diagonal_lcm,
                           source[source_num_rows].expr.get(dim));
    ...
    for (dimension_type dim = 0; dim < dims; ++dim)
    {
        if (dim_kinds[dim] != CON_VIRTUAL)
        {
            ...
            for (dimension_type i = dest_index; i-- > 0; )
            {
                ...
                exact_div_assign(reduced_source_dim, source_dim,
                                 reduced_source_dim);
                multiply_grid(reduced_source_dim, g, dest.sys.rows,
                              dest_num_rows);

                g.expr.exact_div_assign(source_dim, dim, dim + 1);
                // Don't assert g.OK() here, because it may fail.
                // All the rows in `dest' are checked at the end of this
                // function.
            }
        }
    ...
    for (dimension_type dim = 0, i = 0; dim < dims; ++dim)
    {
        if (dim_kinds[dim] != GEN_VIRTUAL)
        {
            ...
            reduce_reduced<Grid_Generator_System>(dest.sys.rows, dim, i++, dim,
                                                  dims - 1, dim_kinds);
        }
    }
