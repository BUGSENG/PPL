    ...
    if (indexes[hint] > key)
    {
        ...
        while (true)
        {
            ...
            if (hint <= offset)
            {
                ...
                // The searched element is in (hint,new_hint) and both indexes
                // point
                // to valid elements.
                break;
            }
...
PPL::CO_Tree::structure_OK () const
{
    ...
    else
    {
        ...
        if (real_size != size_)
        {
            // There are \p real_size elements in the tree that are reachable by
            // the
...
PPL::CO_Tree::OK () const
{
...

    {
        ...
        for (const_iterator itr = begin(), itr_end = end(); itr != itr_end;
             ++itr)
        {
            ++real_size;
        }
    ...
    if (reserved_size > 0)
    {
        ...
        if (is_less_than_ratio(size_, reserved_size, min_density_percent)
            && !is_greater_than_ratio(size_, reserved_size / 2,
                                      max_density_percent))
        {
...
PPL::CO_Tree::rebuild_bigger_tree ()
{
    ...
    dimension_type * const new_indexes = new dimension_type[new_reserved_size +
                                                            2];
...
PPL::dimension_type
PPL::CO_Tree
::compact_elements_in_the_rightmost_end (const dimension_type
                                         last_in_subtree,
                                         dimension_type            subtree_size,
                                         ...
                                         const bool                add_element)
{
    ...
    if (add_element)
    {
        while (subtree_size != 0)
        {
            ...
            if ((last_index_in_subtree == indexes) || (key >
                                                       *last_index_in_subtree))
            {
            ...
            else
            {
                if (last_index_in_subtree != first_unused_index)
                {
                    ...
                    *last_index_in_subtree = unused_index;
                    move_data_element(*first_unused_data,
                                      *last_data_in_subtree);
                }
...
void
PPL::CO_Tree::redistribute_elements_in_subtree (const dimension_type
                                                root_index,
                                                ...
                                                add_element)
{
    ...
    static PPL_TLS std::pair<dimension_type, dimension_type>
...
PPL::CO_Tree::move_data_from (CO_Tree & tree)
{
    ...
    // This is static and with static allocation, to improve performance.
    // sizeof_to_bits(sizeof(dimension_type)) is the maximum k such that 2^k-1
    // is a
    ...
    while (stack_first_empty != 0)
    {
        ...
        const signed char    top_operation = stack[stack_first_empty -
                                                   1].second;
        ...
        else
        {
            ...
            {
                PPL_ASSERT(stack_first_empty + 3 < sizeof(stack) /
                           sizeof(stack[0]));

                const dimension_type half = (top_n + 1) / 2;
                stack[stack_first_empty - 1].second = 0;
                stack[stack_first_empty]            = std::make_pair(top_n -
                                                                     half, 2);
                stack[stack_first_empty + 1]        = std::make_pair(1, 3);
                stack[stack_first_empty + 2].second = 0;
                stack[stack_first_empty + 3]        = std::make_pair(half - 1,
                                                                     1);
                stack_first_empty += 4;
            }
        }
...
PPL::CO_Tree::copy_data_from (const CO_Tree & x)
{
    ...
    catch (...)
    {
        ...
        // successfully.
        // The constructor of data[i] has thrown an exception, so data[i] has
        // not
        // been constructed.
