...
template <typename T, typename Info>
inline Result
set_minus_infinity (const Boundary_Type type, T & x, Info & info, bool
                    open = false)
{
    if (open)
    {
...
template <typename T, typename Info>
inline Result
set_plus_infinity (const Boundary_Type type, T & x, Info & info, bool
                   open = false)
{
    if (open)
    {
...
template <typename T, typename Info>
inline bool
normal_is_boundary_infinity (const Boundary_Type type, const T & x, const
                             Info &)
{
...
is_minus_infinity (const Boundary_Type type, const T & x, const Info & info)
{
    ...
    {
        return !Info::store_special && normal_is_reverse_infinity(type, x,
                                                                  info);
    }
}
...
template <typename T, typename Info>
inline bool
is_boundary_infinity_closed (const Boundary_Type type, const T & x, const
                             Info & info)
{
    return Info::may_contain_infinity
            ...
            const Boundary_Type type, const T & x, const Info & info)
{
    ...
    bool check = (To_Info::check_inexact || (!should_shrink &&
                                             To_Info::store_open));
...
template <typename To, typename To_Info, typename T1, typename Info1, typename
          T2, typename Info2>
inline Result
min_assign (const Boundary_Type to_type, To & to, To_Info & to_info,
...
inline Result
set_zero (const Boundary_Type to_type, To & to, To_Info & to_info, bool
          should_shrink)
{
    bool check = (To_Info::check_inexact || (!should_shrink &&
                                             To_Info::store_open));
    Result r = assign_r(to, Constant<0>::value, round_dir_check(to_type,
                                                                check));
...
mul_assign_z (const Boundary_Type to_type, To & to, To_Info & to_info,
              const Boundary_Type type1, const T1 & x1, const Info1 & info1, int
              x1s,
              const Boundary_Type type2, const T2 & x2, const Info2 & info2, int
              x2s)
{
    bool should_shrink;
    ...
    else
    {
        should_shrink = (info1.get_boundary_property(type1, OPEN)
                         && (x2s != 0 || info2.get_boundary_property(type2,
                                                                     OPEN)));
    }
                  ...
                  unsigned int exp)
{
    ...
    bool check = (To_Info::check_inexact || (!should_shrink &&
                                             To_Info::store_open));
    Result r = umod_2exp_assign_r(to, x, exp, round_dir_check(to_type,
                                                              check));
                  ...
                  unsigned int exp)
{
    ...
    bool check = (To_Info::check_inexact || (!should_shrink &&
                                             To_Info::store_open));
    Result r = smod_2exp_assign_r(to, x, exp, round_dir_check(to_type,
                                                              check));
