    ...
    for (Generator_System::const_iterator k = gs_begin; k != gs_end; ++k)
    {
        ...
        switch (g.type())
        {
            ...
            case Generator::CLOSURE_POINT:
                ...
                if (!mat_initialized)
                {
                    // When handling the first (closure) point, we initialize
                    // the matrix.
                    mat_initialized = true;

                    const Coefficient & d = g.divisor();

                    // TODO: This can be optimized more, if needed, exploiting
                    // the
                    // (possible) sparseness of g. Also consider if OR_Matrix
                    // should be
                    // sparse, too.
                    for (dimension_type i = 0; i < space_dim; ++i)
                    {
                        ...
                        for (dimension_type j = 0; j < i; ++j)
                        {
                            const Coefficient & g_j = g.coefficient(Variable(
                                                                        j));
                            const dimension_type dj = 2 * j;

                            // Set for any point the hyperplanes passing in the
                            // point
                ...
                else
                {
                    ...
                    for (dimension_type i = 0; i < space_dim; ++i)
                    {
                        ...
                        for (dimension_type j = 0; j < i; ++j)
                        {
                            ...
                            // Set for any point the straight lines passing in
                            // the point
                            // and having the octagonal gradient; compute maxima
                            // values.
                            // Let be P = [P_1, P_2, ..., P_n] point.
    ...
    for (Generator_System::const_iterator k = gs_begin; k != gs_end; ++k)
    {
        ...
        switch (g.type())
        {
            case Generator::LINE:
                ...
                // (possible) sparseness of g. Also consider if OR_Matrix should
                // be
                // sparse, too.
                for (dimension_type i = 0; i < space_dim; ++i)
                {
                    ...
                    for (dimension_type j = 0; j < i; ++j)
                    {
                        ...
                        if (g_i != g_j)
                        {
                            ...
                            assign_r(x_ii[dj + 1], PLUS_INFINITY,
                                     ROUND_NOT_NEEDED);
                        }
                        ...
                        {
                            // Hyperplane: X_i + X_j <=/>= +Inf.
                            assign_r(x_i[dj + 1], PLUS_INFINITY,
                                     ROUND_NOT_NEEDED);
    ...
    const Linear_Form< Interval<T, Interval_Info> > & right)
{
    ...
    if (left_t == 0)
    {
        ...
        if (right_t == 1)
        {
            ...
            // Reduce it to the constraint +/-x <= b+ - a- if [c-, c+] = +/-[1,
            // 1].
            ...
            if (right_w_coeff == 1)
            {
                ...
                sub_assign_r(b_plus_minus_a_minus, right_b.upper(),
                             left_a.lower(),
                             ROUND_UP);
                mul_2exp_assign_r(b_plus_minus_a_minus, b_plus_minus_a_minus, 1,
                                  ROUND_UP);
                add_octagonal_constraint(n_right, n_right + 1,
                                         b_plus_minus_a_minus);
                PPL_ASSERT(OK());
                return;
            }
            ...
            if (right_w_coeff == -1)
            {
                ...
                mul_2exp_assign_r(b_plus_minus_a_minus, b_plus_minus_a_minus, 1,
                                  ROUND_UP);
                add_octagonal_constraint(n_right + 1, n_right,
                                         b_plus_minus_a_minus);
                PPL_ASSERT(OK());
                return;
            }
    ...
    else if (left_t == 1)
    {
        if (right_t == 0)
        {
            ...
            // Reduce it to the constraint +/-x <= a+ - b- if [c-, c+] = +/-[1,
            // 1].
            ...
            if (left_w_coeff == 1)
            {
                ...
                sub_assign_r(a_plus_minus_b_minus, right_a.upper(),
                             left_b.lower(),
                             ROUND_UP);
                mul_2exp_assign_r(a_plus_minus_b_minus, a_plus_minus_b_minus, 1,
                                  ROUND_UP);
                add_octagonal_constraint(n_left + 1, n_left,
            ...
            if (left_w_coeff == -1)
            {
                ...
                mul_2exp_assign_r(a_plus_minus_b_minus, a_plus_minus_b_minus, 1,
                                  ROUND_UP);
                add_octagonal_constraint(n_left, n_left + 1,
                                         a_plus_minus_b_minus);
                PPL_ASSERT(OK());
                return;
            }
        ...
        if (right_t == 1)
        {
            ...
            if (left_w_id == right_w_id)
            {
                ...
                if (is_left_coeff_one && is_right_coeff_minus_one)
                {
                    ...
                    const FP_Interval_Type & left_b =
                        left.inhomogeneous_term();
                    const FP_Interval_Type & right_a =
                        right.inhomogeneous_term();
            ...
            else if (is_left_coeff_one && is_right_coeff_one)
            {
                ...
                sub_assign_r(c_plus_minus_a_minus, right_c.upper(),
                             left_a.lower(),
                             ROUND_UP);

                if (left_w_id < right_w_id)
                {
                    add_octagonal_constraint(n_right, n_left,
            ...
            if (is_left_coeff_one && is_right_coeff_minus_one)
            {
                ...
                if (left_w_id < right_w_id)
                {
                    add_octagonal_constraint(n_right + 1, n_left,
                                             c_plus_minus_a_minus);
                }
                else
                {
                    add_octagonal_constraint(n_left + 1, n_right,
                                             c_plus_minus_a_minus);
                }
            ...
            if (is_left_coeff_minus_one && is_right_coeff_one)
            {
                ...
                if (left_w_id < right_w_id)
                {
                    add_octagonal_constraint(n_right, n_left + 1,
                                             c_plus_minus_a_minus);
                }
                else
                {
                    add_octagonal_constraint(n_left, n_right + 1,
                                             c_plus_minus_a_minus);
                }
            ...
            if (is_left_coeff_minus_one && is_right_coeff_minus_one)
            {
                ...
                {
                    add_octagonal_constraint(n_left, n_right,
                                             c_plus_minus_a_minus);
                }
    ...
    for (dimension_type first_v = 0; first_v < max_w_id; ++first_v)
    {
             ...
             second_v <= max_w_id; ++second_v)
        {
            ...
            if ((low_coeff != 0) || (high_coeff != 0))
            {
                ...
                {
                    assign_r(low_coeff, rsv_coefficient.lower(),
                             ROUND_NOT_NEEDED);
                    assign_r(high_coeff, rsv_coefficient.upper(),
                             ROUND_NOT_NEEDED);
            ...
            else
            {
                ...
                if ((low_coeff != 0) || (high_coeff != 0))
                {
                    assign_r(low_coeff, lsv_coefficient.lower(),
                             ROUND_NOT_NEEDED);
                    assign_r(high_coeff, lsv_coefficient.upper(),
                             ROUND_NOT_NEEDED);
            ...
            if (do_update)
            {
                ...
                linear_form_upper_bound(right_minus_left + first + second,
                                        upper_bound);
                add_octagonal_constraint(n_second_var + 1, n_first_var,
                                         upper_bound);
                linear_form_upper_bound(right_minus_left - first - second,
                                        upper_bound);
                add_octagonal_constraint(n_second_var, n_first_var + 1,
                                         upper_bound);
                linear_form_upper_bound(right_minus_left + first - second,
                                        upper_bound);
                add_octagonal_constraint(n_second_var, n_first_var,
                                         upper_bound);
            }
        }
    }
    ...
    for (dimension_type v = 0; v <= max_w_id; ++v)
    {
        ...
        if (do_update)
        {
            ...
            /*
               ...
               while computing the new ones, we momentarily keep the new
                  coefficients
...
Octagonal_Shape<T>::contains_integer_point () const
{
    ...
    typename OR_Matrix<N>::const_element_iterator x_i =
        matrix.element_begin();
...
Octagonal_Shape<T>::constrains (const Variable var) const
{
    ...
    const dimension_type n_v = 2 *
                               (var_space_dim - 1);

    typename OR_Matrix<N>::const_row_iterator m_iter =
        matrix.row_begin() + n_v;
...
Octagonal_Shape<T>::is_strong_coherent () const
{
    ...
    {
        typename OR_Matrix<N>::const_row_iterator iter =
            matrix.row_begin() + i;
                            ...
                            const bool                from_above) const
{
                                       ...
                                       i, j, coeff, term))
    {
        ...
        typename OR_Matrix<N>::const_row_iterator i_iter =
            matrix.row_begin() + i;
                             ...
                             bool & included) const
{
    ...
    else
    {
        ...
        if (!is_plus_infinity(m_i[j]))
        {
            ...
            // Set `coeff_expr' to the absolute value of coefficient of a
            // variable
            // of `expr'.
...
Octagonal_Shape<T>::relation_with (const Generator & g) const
{
      ...
      : g.coefficient(x);
        ...
        else
        {
            ...
            if (!is_plus_infinity(m_ii_i))
            {
                ...
                {
                    // If the generator is not a line it's necessary to check
                    // that the scalar product sign is not positive and the
                    // scalar
                    // product has the form
                    // 'denom * g_coeff_x - numer * g.divisor()'.
                    if (product > 0)
    ...
    for (row_iterator i_iter = m_begin ; i_iter != m_end; i_iter += 2)
    {
        ...
        : g.coefficient(y);
            ...
            const bool difference_is_equality = is_additive_inverse(m_ii_jj,
                                                                    m_i_j);
            ...
            if (difference_is_equality)
            {
                ...
                // To satisfy the constraint it's necessary that the scalar
                // product
                // is not zero.
                numer_denom(m_i_j, numer, denom);
                product = denom * g_coeff_x;
            ...
            else
            {
                if (!is_plus_infinity(m_i_j))
                {
                    // The constraint has form a*x - a*y <= b.
                    // The scalar product has the form
                    // 'denom * coeff_x - denom * coeff_y - numer *
                    // g.divisor()'.
                    // If the generator is not a line it's necessary to check
...
::incremental_strong_closure_assign (const Variable var) const
{
    ...
    for (row_iterator k_iter = m_begin; k_iter != m_end; ++k_iter)
    {
        ...
        row_reference        x_ck = (k % 2 != 0) ? *(k_iter - 1) : *(k_iter +
                                                                     1);
        ...
        for (row_iterator i_iter = m_begin; i_iter != m_end; ++i_iter)
        {
            ...
            row_reference        x_ci = (i % 2 != 0) ? *(i_iter - 1) :
                                        *(i_iter + 1);
        ...
        for (dimension_type lj = 0 ; lj <= rs_li; ++lj)
        {
            ...
            for (dimension_type lk = 0; lk < num_no_sing_leaders; ++lk)
            {
                ...
                if ((k != i) && (k != j))
                {
                    ...
                    {
                        // Case 3.
                        add_assign_r(tmp, matrix[ck][ci], matrix[k][j],
                                     ROUND_UP);
                    }
...
Octagonal_Shape<T>::remove_space_dimensions (const Variables_Set & vars)
{
    ...
    Elem_Iter            iter       = matrix.element_begin() +
                                      static_cast<diff_t>(first_size);

    for (dimension_type i = first + 1; i < space_dim; ++i)
    {
        if (vars.count(i) == 0)
        {
            typename OR_Matrix<N>::row_iterator row_iter =
                matrix.row_begin() + 2 * i;
                                       ...
                                       unsigned                * tp)
{
    ...
    {
        throw_constraint_incompatible(
            "limited_CH78_extrapolation_assign(y, cs)");
    }
...
Octagonal_Shape<T>::CC76_narrowing_assign (const Octagonal_Shape & y)
{
    ...
    typename OR_Matrix<N>::const_element_iterator j =
        y.matrix.element_begin();
         ...
         u_end = sc_expr.lower_bound(Variable(last_id + 1)); u != u_end; ++u)
    {
        ...
        if (expr_u > 0)
        {
            if (expr_u >= sc_denom)
            {
                ...
                N & m_v_minus_u = (n_v < n_u) ? matrix[n_u][n_v] : m_cv[n_u +
                                                                        1];
            ...
            else
            {
                ...
                if (!is_plus_infinity(m_u_cu))
                {
                    ...
                    div_2exp_assign_r(minus_lb_u, minus_lb_u, 1,
                                      ROUND_NOT_NEEDED);
        ...
        else
        {
            ...
            {
                // Here -1 < q < 0.
                typename OR_Matrix<N>::row_reference_type m_cu = matrix[n_u +
                                                                        1];
                ...
                if (!is_plus_infinity(m_cu_u))
                {
                    ...
                    div_assign_r(minus_q, minus_q, mpq_sc_denom,
                                 ROUND_NOT_NEEDED);
         ...
         u_end = sc_expr.lower_bound(Variable(last_id + 1)); u != u_end; ++u)
    {
        ...
        if (expr_u > 0)
        {
            if (expr_u >= sc_denom)
            {
                ...
                N & m_u_minus_v = (n_v < n_u) ? matrix[n_u + 1][n_v + 1] :
                                  m_v[n_u];
            ...
            else
            {
                ...
                if (!is_plus_infinity(m_cu_u))
                {
                    ...
                    assign_r(q, expr_u, ROUND_NOT_NEEDED);
                    div_assign_r(q, q, mpq_sc_denom, ROUND_NOT_NEEDED);
                    assign_r(minus_lb_u, matrix[n_u][n_u + 1],
                             ROUND_NOT_NEEDED);
                    div_2exp_assign_r(minus_lb_u, minus_lb_u, 1,
                                      ROUND_NOT_NEEDED);
                    // Compute `ub_u - lb_u'.
                    add_assign_r(minus_lb_u, ub_u, minus_lb_u,
                                 ROUND_NOT_NEEDED);
          ...
          : m_v[n_u + 1];
            ...
            else
            {
                ...
                if (!is_plus_infinity(m_u_cu))
                {
                    ...
                    // Deducing `-v - u <= -lb_v - ((-q) * ub_u + (1 + q) *
                    // lb_u)'.
                    N & m_minus_v_minus_u = (n_v < n_u) ? m_u[n_v + 1] :
                                            m_v[n_u + 1];

                    add_assign_r(m_minus_v_minus_u, minus_lb_v, up_approx,
                                 ROUND_UP);
                }
            }
        }
            ...
            case EQUAL:
                ...
                if (w_coeff == denominator)
                {
                    ...
                    if (var_id < w_id)
                    {
                        ...
                        add_octagonal_constraint(n_w + 1, n_var + 1, b,
                                                 minus_denom);
                    }
                    else
                    {
                        add_octagonal_constraint(n_var + 1, n_w + 1, b,
                                                 denominator);
                ...
                else
                {
                    ...
                    if (var_id < w_id)
                    {
                        add_octagonal_constraint(n_w + 1, n_var, b,
                                                 denominator);
                        add_octagonal_constraint(n_w, n_var + 1, b,
                                                 minus_denom);
                    }
                    else
                    {
                        add_octagonal_constraint(n_var + 1, n_w, b,
                                                 denominator);
                        add_octagonal_constraint(n_var, n_w + 1, b,
                                                 minus_denom);
                    }
                }
            ...
            case EQUAL:
                {
                    ...
                    // Note: indices above `w' can be disregarded, as they all
                    // have
                         ...
                         m_iter != m_iter_end; )
                    {
                        ...
                        const Coefficient & sc_i = sc_expr.coefficient(
                            Variable(id));
                        ...
                        if (sign_i > 0)
                        {
                            ...
                            if (pinf_count <= 1)
                            {
                                ...
                                if (!is_plus_infinity(double_approx_i))
                                {
                                    ...
                                    div_2exp_assign_r(half, double_approx_i, 1,
                                                      ROUND_UP);
                                    add_mul_assign_r(sum, coeff_i, half,
                                                     ROUND_UP);
                                }
                                else
                                {
                            ...
                            if (neg_pinf_count <= 1)
                            {
                                ...
                                if (!is_plus_infinity(double_approx_minus_i))
                                {
                                    ...
                                    div_2exp_assign_r(half,
                                                      double_approx_minus_i, 1,
                                                      ROUND_UP);
                                    add_mul_assign_r(neg_sum, coeff_i, half,
                                                     ROUND_UP);
                                }
                                else
                                {
                        ...
                        else if (sign_i < 0)
                        {
                            ...
                            if (pinf_count <= 1)
                            {
                                ...
                                if (!is_plus_infinity(double_approx_minus_i))
                                {
                                    ...
                                    div_2exp_assign_r(half,
                                                      double_approx_minus_i, 1,
                                                      ROUND_UP);
                                    add_mul_assign_r(sum, minus_coeff_i, half,
                                                     ROUND_UP);
                                }
                                else
                                {
                            ...
                            if (neg_pinf_count <= 1)
                            {
                                ...
                                if (!is_plus_infinity(double_approx_i))
                                {
                                    ...
                                    div_2exp_assign_r(half, double_approx_i, 1,
                                                      ROUND_UP);
                                    add_mul_assign_r(neg_sum, minus_coeff_i,
                                                     half, ROUND_UP);
                                }
                                else
                                {
                    ...
                    if (pinf_count <= 1)
                    {
                        ...
                        if (sc_denom != 1)
                        {
                            // Before computing quotients, the denominator
                            // should be
                            // approximated towards zero. Since `sc_denom' is
                            // known to be
                            // positive, this amounts to rounding downwards,
                            // which is
                            ...
                            neg_assign_r(down_sc_denom, down_sc_denom,
                                         ROUND_UP);
                        ...
                        if (pinf_count == 0)
                        {
                            ...
                            // Deduce constraints of the form `v +/- u', where
                            // `u != v'.
                            deduce_v_pm_u_bounds(var_id, w_id, sc_expr,
                                                 sc_denom, sum);
                        }
                        else
                        {
                            ...
                            if (pinf_index != var_id)
                            {
                                ...
                                if (ppi == sc_denom)
                                {
                                    // Add the constraint `v - pinf_index <=
                                    // sum'.
                                ...
                                else
                                {
                                    if (ppi == minus_sc_denom)
                                    {
                                        ...
                                        if (var_id < pinf_index)
                                        {
                                            matrix[2 * pinf_index + 1][n_var] =
                                                sum;
                                        }
                                        else
                                        {
                                            matrix[n_var + 1][2 * pinf_index] =
                                                sum;
                                        }
                                    }
                                }
                    ...
                    if (neg_pinf_count <= 1)
                    {
                        ...
                        if (sc_denom != 1)
                        {
                            ...
                            neg_assign_r(down_sc_denom, down_sc_denom,
                                         ROUND_UP);
                            div_assign_r(neg_sum, neg_sum, down_sc_denom,
                                         ROUND_UP);
                        }
                        ...
                        if (neg_pinf_count == 0)
                        {
                            // Add the constraint `v >= -neg_sum', i.e., `-v <=
                            // neg_sum'.
                            PPL_DIRTY_TEMP(N, double_neg_sum);
                            mul_2exp_assign_r(double_neg_sum, neg_sum, 1,
                                              ROUND_UP);
                            matrix[n_var][n_var + 1] = double_neg_sum;
                            // Deduce constraints of the form `-v +/- u', where
                            // `u != v'.
                            deduce_minus_v_pm_u_bounds(var_id, w_id, sc_expr,
                                                       sc_denom,
                                                       neg_sum);
                        }
                        else
                        {
                            ...
                            if (neg_pinf_index != var_id)
                            {
                                const Coefficient & npi
                                    = sc_expr.coefficient(Variable(
                                                              neg_pinf_index));

                                if (npi == sc_denom)
                                {
                                    // Add the constraint `v - neg_pinf_index >=
                                    // -neg_sum',
                                    // i.e., `neg_pinf_index - v <= neg_sum'.
                                    if (neg_pinf_index < var_id)
                                    {
                                        matrix[n_var][2 * neg_pinf_index] =
                                            neg_sum;
                                    }
                                    else
                                    {
                                        matrix[2 * neg_pinf_index + 1][n_var +
                                                                       1] =
                                            neg_sum;
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        // Add the constraint `v +
                                        // neg_pinf_index >= -neg_sum',
                                        // i.e., `-neg_pinf_index - v <=
                                        // neg_sum'.
                                        if (neg_pinf_index < var_id)
                                        {
                                            matrix[n_var][2 * neg_pinf_index +
            ...
            case LESS_OR_EQUAL:
                {
                    ...
                    // Note: indices above `w_id' can be disregarded, as they
                    // all have
                         ...
                         m_iter != m_iter_end; )
                    {
                        ...
                        const N & double_approx_i = (sign_i > 0) ? m_ci[n_i] :
                                                    m_i[n_i + 1];
                        ...
                        div_2exp_assign_r(approx_i, double_approx_i, 1,
                                          ROUND_UP);
                    ...
                    if (sc_denom != 1)
                    {
                        // Before computing the quotient, the denominator should
                        // be
                        // approximated towards zero. Since `sc_denom' is known
                        // to be
                        // positive, this amounts to rounding downwards, which
                        // is achieved
                        // by rounding upwards `minus_sc-denom' and negating
                        // again the result.
                        PPL_DIRTY_TEMP(N, down_sc_denom);
                        assign_r(down_sc_denom, minus_sc_denom, ROUND_UP);
                    ...
                    if (pinf_count == 0)
                    {
                        ...
                        // Deduce constraints of the form `v +/- u', where `u !=
                        // v'.
                        deduce_v_pm_u_bounds(var_id, w_id, sc_expr, sc_denom,
                                             sum);
                    }
                    else if (pinf_count == 1)
                    {
                        ...
                        if (expr.coefficient(Variable(pinf_index)) ==
                            denominator)
                        {
                            ...
                            {
                                add_octagonal_constraint(n_var + 1, pinf_ind +
                                                         1, sum);
                            }
                        }
                        else
                        {
                                ...
                                minus_denom)
                            {
                                ...
                                if (var_id < pinf_index)
                                {
                                    add_octagonal_constraint(pinf_ind + 1,
                                                             n_var, sum);
                                }
                                else
                                {
                                    add_octagonal_constraint(n_var + 1,
                                                             pinf_ind, sum);
                                }
                            }
                        }
            ...
            case GREATER_OR_EQUAL:
                {
                    ...
                    // Note: approximating `-sc_expr' from above and then
                    // negating the
                         ...
                         m_iter != m_iter_end; )
                    {
                        ...
                        // Choose carefully: we are approximating `-sc_expr'.
                        const N & double_approx_i = (sign_i > 0) ? m_i[n_i +
                                                                       1] :
                                                    m_ci[n_i];
                    ...
                    if (pinf_count == 0)
                    {
                        ...
                        // Deduce constraints of the form `-v +/- u', where `u
                        // != v'.
                        deduce_minus_v_pm_u_bounds(var_id, pinf_index, sc_expr,
                                                   sc_denom,
                                                   sum);
                    }
                    else if (pinf_count == 1)
                    {
                            ...
                            denominator)
                        {
                            ...
                            {
                                add_octagonal_constraint(pinf_ind + 1, n_var,
                                                         sum);
                            }
                        }
                        else
                        {
                                ...
                                minus_denom)
                            {
                                ...
                                if (pinf_index < var_id)
                                {
                                    add_octagonal_constraint(n_var, pinf_ind +
                                                             1, sum);
                                }
                                else
                                {
                                    add_octagonal_constraint(pinf_ind, n_var +
                                                             1, sum);
                                }
                            }
                        }
                                  ...
                                  denominator)
{
        ...
        if (sign_i > 0)
        {
            ...
            if (neg_pinf_count <= 1)
            {
                ...
                if (!is_plus_infinity(double_up_approx_minus_i))
                {
                    ...
                    div_2exp_assign_r(half, double_up_approx_minus_i, 1,
                                      ROUND_UP);
    ...
    if (pos_pinf_count <= 1)
    {
        ...
        if (sc_denom != 1)
        {
            ...
            // approximated
            // towards zero. Since `sc_denom' is known to be positive, this
            // amounts to
        ...
        else
        {
            ...
            {
                const Coefficient & ppi = sc_expr.coefficient(Variable(
                                                                  pos_pinf_index));
    ...
    if (neg_pinf_count <= 1)
    {
        ...
        else
        {
            ...
            if (neg_pinf_index != var_id)
            {
                const Coefficient & npi = sc_expr.coefficient(Variable(
                                                                  neg_pinf_index));
...
template <typename Interval_Info>
void
Octagonal_Shape<T>::affine_form_image (const Variable
                                       var,
         ...
         ++curr_var)
    {
        ...
        if ((curr_lb != 0) || (curr_ub != 0))
        {
            ...
            neg_assign_r(curr_minus_var_ub, matrix[n_var][n_var + 1],
                         ROUND_NOT_NEEDED);
            div_2exp_assign_r(curr_minus_var_ub, curr_minus_var_ub, 1,
                              ROUND_DOWN);

            // Optimize the most common case: curr = +/-[1, 1].
            if ((curr_lb == 1) && (curr_ub == 1))
            {
                add_assign_r(result, result, std::max(curr_var_ub,
                                                      curr_minus_var_ub),
                             ROUND_UP);
            }
            ...
            else
            {
                ...
                assign_r(first_comparison_term, std::max(first_comparison_term,
                         ...
                         ROUND_NOT_NEEDED);
                assign_r(second_comparison_term, 0, ROUND_NOT_NEEDED);
                add_mul_assign_r(second_comparison_term, curr_minus_var_ub,
                                 curr_ub,
                                 ROUND_UP);
                assign_r(first_comparison_term, std::max(first_comparison_term,
                                                         second_comparison_term),
                                  ...
                                  N & result)
{
    ...
    assign_r(result, std::max(result, second_comparison_term),
             ROUND_NOT_NEEDED);
    assign_r(result, std::max(result, third_comparison_term), ROUND_NOT_NEEDED);
    assign_r(result, std::max(result, fourth_comparison_term),
             ROUND_NOT_NEEDED);
}
                                     ...
                                     denominator)
{
    ...
    {
        // The transformation is not invertible: all constraints on `var' are
        // lost.
        forget_all_octagonal_constraints(var_id);
        PPL_ASSERT(OK());
    }
                            ...
                            Coefficient_traits::const_reference denominator)
{
                ...
                case LESS_OR_EQUAL:
                    {
                        ...
                        if (w_id == var_id)
                        {
                            // Here `expr' is of the form: +/- denominator * v +
                            // b.
                            ...
                            if (w_coeff == denominator)
                            {
                                // Translate all the constraints of the form `v
                                // - w <= cost'
                                // into the constraint `v - w <= cost +
                                // b/denominator';
                                ...
                                for ( ; m_iter != m_end; ++m_iter)
                                {
                                    ...
                                    add_assign_r(m_i_v, m_i_v, d, ROUND_UP);
                                    assign_r(m_i[n_var + 1], PLUS_INFINITY,
                                             ROUND_NOT_NEEDED);
                                }

                                for (dimension_type k = n_var; k-- > 0; )
                                {
                                    assign_r(m_v[k], PLUS_INFINITY,
                                             ROUND_NOT_NEEDED);
                            ...
                            else
                            {
                                ...
                                mul_2exp_assign_r(d, d, 1, ROUND_UP);
                                add_assign_r(matrix[n_var + 1][n_var], m_v_cv,
                                             d, ROUND_UP);
                                assign_r(m_v_cv, PLUS_INFINITY,
                                         ROUND_NOT_NEEDED);
                        ...
                        else
                        {
                            ...
                            if (w_coeff == denominator)
                            {
                                // Add the new constraint `v - w <=
                                // b/denominator'.
                                if (var_id < w_id)
                                {
                                    add_octagonal_constraint(n_w, n_var, b,
                                                             denominator);
                                }
                                else
                                {
                                    add_octagonal_constraint(n_var + 1, n_w + 1,
                                                             b, denominator);
                                }
                            }
                            else
                ...
                case GREATER_OR_EQUAL:
                    {
                        ...
                        if (w_id == var_id)
                        {
                            ...
                            if (w_coeff == denominator)
                            {
                                ...
                                for ( ; m_iter != m_end; ++m_iter)
                                {
                                    ...
                                    assign_r(m_i[n_var], PLUS_INFINITY,
                                             ROUND_NOT_NEEDED);
                                    add_assign_r(m_i[n_var + 1], m_i[n_var + 1],
                                                 d, ROUND_UP);
                                }
                                ...
                                for (dimension_type k = n_var; k-- > 0; )
                                {
                                    ...
                                    assign_r(m_cv[k], PLUS_INFINITY,
                                             ROUND_NOT_NEEDED);
                                }

                                mul_2exp_assign_r(d, d, 1, ROUND_UP);
                                add_assign_r(m_v_cv, m_v_cv, d, ROUND_UP);
                                assign_r(m_cv_v, PLUS_INFINITY,
                                         ROUND_NOT_NEEDED);
                            }
                            else
                            {
                                ...
                                mul_2exp_assign_r(d, d, 1, ROUND_UP);
                                add_assign_r(matrix[n_var][n_var + 1], m_cv_v,
                                             d, ROUND_UP);
                                assign_r(m_cv_v, PLUS_INFINITY,
                                         ROUND_NOT_NEEDED);
                                forget_binary_octagonal_constraints(var_id);
                        ...
                        else
                        {
                            ...
                            // We remove all constraints of the form `var (+/-
                            // var1) >= const'
                            // and we add the new constraint `var +/- var1 >=
                            // n/denominator'.
                            if (w_coeff == denominator)
                            {
                                // Add the new constraint `var - w >=
                                // b/denominator',
                                // i.e., `w - var <= -b/denominator'.
                                if (var_id < w_id)
                                {
                                    add_octagonal_constraint(n_w + 1, n_var + 1,
                                                             b, minus_denom);
                                }
                                else
                                {
                                    add_octagonal_constraint(n_var, n_w, b,
                                                             minus_denom);
                                }
                            }
                            else
    ...
    switch (relsym)
    {
        case LESS_OR_EQUAL:
            {
                ...
                // Note: indices above `w' can be disregarded, as they all have
                // a zero coefficient in `sc_expr'.
                for (Row_iterator m_iter = m_begin, m_iter_end = m_begin + (2 *
                                                                            w_id
                                                                            + 2);
                     m_iter != m_iter_end; )
                {
                    ...
                    const Coefficient & sc_i = sc_expr.coefficient(Variable(
                                                                       id));
                    const int sign_i = sgn(sc_i);
                ...
                if (sc_denom != 1)
                {
                    ...
                    // approximated towards zero. Since `sc_denom' is known to
                    // be
                ...
                else if (pinf_count == 1)
                {
                    if (pinf_index != var_id)
                    {
                        const Coefficient & pi = expr.coefficient(Variable(
                                                                      pinf_index));
        ...
        case GREATER_OR_EQUAL:
            {
                // Compute an upper approximation for `-sc_expr' into `sum'.
                // Note: approximating `-sc_expr' from above and then negating
                // the
                // result is the same as approximating `sc_expr' from below.
                     ...
                     m_iter != m_iter_end; )
                {
                    ...
                    // Choose carefully: we are approximating `-sc_expr'.
                    const N & double_approx_i = (sign_i > 0) ? m_i[n_i + 1] :
                                                m_ci[n_i];
                                              ...
                                              const Linear_Expression & rhs)
{
    ...
    {
        // `lhs' is a constant.
        // In principle, it is sufficient to add the constraint `lhs relsym
        // rhs'.
    ...
    else
    {
        ...
        for (Linear_Expression::const_iterator i = lhs.begin(), i_end =
                 lhs.end();
             i != i_end; ++i)
        {
        ...
        const dimension_type num_common_dims = std::min(lhs_space_dim,
                                                        rhs_space_dim);

        if (!lhs.have_a_common_variable(rhs, Variable(0), Variable(
                                            num_common_dims)))
        {
            ...
            // it will be simply ignored. Should we compute approximations for
            // it?
            switch (relsym)
            {
                case LESS_OR_EQUAL:
    ...
    if (expr_v != 0)
    {
      ...
      ? GREATER_OR_EQUAL : LESS_OR_EQUAL;
        ...
        const Relation_Symbol inverse_relsym
            = (sgn(denominator) == sgn(inverse_denom)) ? relsym :
              reversed_relsym;
                               ...
                               const Linear_Expression & rhs)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // Constrain the new dimension so that it is related to
            // the left hand side as dictated by `relsym'.
            // Note: if `rhs == v + b_rhs' or `rhs == -v + b_rhs' or `rhs ==
            // b_rhs',
                                             ...
                                             denominator)
{
    ...
    if (denominator == 0)
    {
        throw_invalid_argument("bounded_affine_preimage(v, lb, ub, d)",
                               "d == 0");
    }
...
Octagonal_Shape<T>::expand_space_dimension (Variable var, dimension_type m)
{
    ...
    {
        throw_dimension_incompatible("expand_space_dimension(v, m)", var_id +
                                     1);
    }
                                           ...
                                           Variable              dest)
{
         ...
         vs_end = vars.end(); i != vs_end; ++i)
    {
        ...
        for (dimension_type j = min_id + 2; j < max_id; ++j)
        {
            ...
            row_reference        m_cj   = (j % 2 != 0) ? *(j_iter - 1) :
                                          *(j_iter + 1);
...
::integer_upper_bound_assign_if_exact (const Octagonal_Shape & y)
{
    ...
    if (x_space_dim != y.space_dimension())
    {
        throw_dimension_incompatible("integer_upper_bound_assign_if_exact(y)",
                                     y);
    }
        ...
        for (dimension_type j = row_size_i; j-- > 0; )
        {
          ...
          : ((ck < row_size_i) ? ub_i[ck] : ub_k[ci]);
                ...
                for (dimension_type ell = row_size_k; ell-- > 0; )
                {
                    ...
                    const dimension_type cell      = coherent_index(ell);
                    const N            & eps_k_ell = (k == cell) ? temp_two :
                                                     temp_one;

                    // Check condition 1b in BHZ09 Theorem 6.8.
                    add_assign_r(lhs_k_ell, ty_k_ell, eps_k_ell,
                                 ROUND_NOT_NEEDED);
...
IO_Operators::operator<< (std::ostream & s, const Octagonal_Shape<T> & oct)
{
    ...
    for (row_iterator i_iter = m_begin; i_iter != m_end; )
    {
        ...
        for (dimension_type j = 0; j < i; j += 2)
        {
            ...
            {
                // It is an equality.
                PPL_ASSERT(!is_plus_infinity(c_i_j) && !is_plus_infinity(
                               c_ii_jj));
            ...
            {
                // It is an equality.
                PPL_ASSERT(!is_plus_infinity(c_i_jj) && !is_plus_infinity(
                               c_ii_j));
...
template <typename T>
void
Octagonal_Shape<T>::throw_expression_too_complex (const
                                                  char *
                                                  method,
