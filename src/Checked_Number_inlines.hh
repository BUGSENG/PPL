...
template <typename From>
inline
Checked_Number<T, Policy>::Checked_Number (const From &, typename
                                           Enable_If<Is_Special<From>::value,
                                                     bool>::type)
{
    Rounding_Dir dir = Policy::ROUND_DEFAULT_CONSTRUCTOR;
...
{
    return check_result(Checked::assign_special<typename
                                                Native_Checked_To_Wrapper<To>
                                                ::Policy>(
                            Native_Checked_To_Wrapper<To>::raw_value(to),
                            From::vclass,
                            rounding_dir(dir)),
                        dir);
}
...
{
    return check_result(Checked::construct_special<typename
                                                   Native_Checked_To_Wrapper<To>
                                                   ::Policy>(
                            Native_Checked_To_Wrapper<To>::raw_value(to),
                            From::vclass,
...
is_minus_infinity (const T & x)
{
    return Checked::is_minf<typename Native_Checked_From_Wrapper<T>
                            ::Policy>(Native_Checked_From_Wrapper<T>::raw_value(
                                          x));
}
...
#define PPL_DEFINE_FUNC2(name, func)                                           \
    ...
    PPL_U(name)(To & to, const From1 & x, const From2 & y, Rounding_Dir dir) { \
        return                                                                 \
            check_result(Checked::func<typename Native_Checked_To_Wrapper<To>  \
                                       ::Policy,                               \
                                       typename Native_Checked_From_Wrapper<   \
                                           From1>                              \
                                       ::Policy,                               \
                                       typename Native_Checked_From_Wrapper<   \
                                           From2>                              \
...
#define PPL_DEFINE_FUNC4(name, func)                                             \
    ...
    PPL_U(name)(To1 & to, To2 & s, To3 & t, const From1 & x, const From2 & y,    \
                Rounding_Dir dir) {                                              \
        return                                                                   \
            check_result(Checked::func<typename                                  \
                                       Native_Checked_To_Wrapper<To1>::Policy,   \
                                       typename Native_Checked_To_Wrapper<To2>:: \
                                       Policy,                                   \
                                       typename Native_Checked_To_Wrapper<To3>:: \
                                       Policy,                                   \
                                       typename Native_Checked_From_Wrapper<     \
                                           From1>::Policy,                       \
                                       typename Native_Checked_From_Wrapper<     \
...
#define PPL_DEFINE_ASSIGN_FUN2_2(f, fun)                                        \
    template <typename T, typename Policy>                                      \
    inline void                                                                 \
    PPL_U(f)(Checked_Number<T, Policy> & x, const Checked_Number<T,             \
                                                                 Policy> & y) { \
...
#define PPL_DEFINE_ASSIGN_FUN5_5(f, fun)                                   \
    ...
    PPL_U(f)(Checked_Number<T, Policy> & x,                                \
             ...
             const Checked_Number<T, Policy> & y,                          \
             const Checked_Number<T, Policy> & z) {                        \
        Policy::handle_result((fun) (x, s, t, y, z,                        \
                                     Policy::ROUND_DEFAULT_FUNCTION));     \
    }
...
#define PPL_DEFINE_ASSIGN_2EXP(f, fun)                                 \
    ...
    PPL_U(f)(Checked_Number<T, Policy> & x,                            \
        ...
        Policy::handle_result((fun) (x, y, exp,                        \
                                     Policy::ROUND_DEFAULT_FUNCTION)); \
    }
...
{
    return check_result(Checked::output_ext<typename
                                            Native_Checked_From_Wrapper<T>::
                                            Policy>(os,
                                                    Native_Checked_From_Wrapper
                                                    <
                                                        T>::raw_value(x),
...
{
    return check_result(Checked::input_ext<typename
                                           Native_Checked_To_Wrapper<T>::Policy>(
