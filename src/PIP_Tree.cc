 ...
 */
void
find_lexico_minimal_column_in_set (
    ...
    const PIP_Tree_Node::Row          & pivot_row)
{
    ...
    for (dimension_type var_index = 0; var_index < num_vars; ++var_index)
    {
        ...
        else
        {
            ...
            PIP_Tree_Node::Row::const_iterator row_itr = row.lower_bound(
                min_column);
    ...
    for (dimension_type var_index = 0; var_index < num_vars; ++var_index)
    {
        ...
        if (in_base)
        {
            for (++i; i != i_end; ++i)
            {
                ...
                Coefficient_traits::const_reference challenger_cost =
                    i->second.cost;
        ...
        else
        {
            ...
            for (++i; i != i_end; ++i)
            {
                ...
                else
                {
                    ...
                    // Using end() as a hint is not useful, keep the current
                    // hint.
                }

                PPL_ASSERT(row_challenger_value == row.get(challenger_j));

                // Before computing and comparing the actual values, the signs
                // are
                // compared. This speeds up the code, because the values'
                // computation
                // is a bit expensive.

                const int lhs_sign = sgn(cost) * sgn(row_value);
                const int rhs_sign = sgn(challenger_cost) * sgn(
                    row_challenger_value);
...
PIP_Decision_Node::OK () const
{
    ...
    if (false_child != nullptr)
    {
        const dimension_type dist = Implementation::num_constraints(
            constraints_);
                          ...
                          const int             indent_level)
{
    ...
    if (has_false_child)
    {
        ...
        // NOTE: modify context_true in place, complementing its last
        // constraint.
        Matrix<Row> & context_false = context_true;
        Row         & last          = context_false[context_false.num_rows() -
                                                    1];
...
PIP_Decision_Node::ascii_load (std::istream & s)
{
    ...
    {
        PIP_Decision_Node * const dec = new PIP_Decision_Node(nullptr, nullptr,
                                                              nullptr);
...
bool
PIP_Tree_Node::compatibility_check (const Matrix<Row> & context, const
                                    Row & row)
{
                 ...
                 j = pivot.begin(), j_end = pivot.end(); j != j_end; ++j)
            {
                ...
                for (dimension_type i = num_rows; i-- > 0; )
                {
                    ...
                    if (product % pivot_pj != 0)
                    {
                        ...
                        for (Row::iterator
                             k = s_i.begin(), k_end = s_i.end(); k != k_end;
                             ++k)
                        {
                            WEIGHT_ADD(30);
                            *k *= scale_factor;
                  ...
                  const Variables_Set       & parameters)
{
    ...
    for (dimension_type i = initial_space_dim; i < external_space_dim; ++i)
    {
        if (parameters.count(i) == 0)
        {
            ...
            else
            {
                ...
                mapping.insert(nth_iter(mapping, new_var_column),
                               new_var_column);
        ...
        && (pip.big_parameter_dimension != not_a_dimension()))
    {
        ...
        big_dimension = 1U
                        + static_cast<dimension_type>(std::distance(
                                                          parameters.begin(),
                                                          pos));
    }
        ...
        else
        {
            ...
            if (constraint.is_equality())
            {
                ...
                if ((special_equality_row == 0) || basis[special_equality_row])
                {
                    ...
                    // FIXME: for now, we do not handle the case where the
                    // variable
                    // is basic, and we just create a new row.
                    // This might be faster however.
                    tableau.s.add_zero_rows(1);
                    tableau.t.add_zero_rows(1);
                    // NOTE: addition of rows invalidates references v_row and
                    // p_row
                    ...
                    sign.push_back(row_sign(tableau.t[1 + row_id],
                                            big_dimension));
                          ...
                          const int             indent_level)
{
    ...
    while (true)
    {
        ...
        const dimension_type num_rows =
            tableau.t.num_rows();
        const dimension_type num_vars =
            tableau.s.num_columns();
        const dimension_type num_params =
            tableau.t.num_columns();
        Coefficient_traits::const_reference tableau_denom =
            tableau.denominator();
        ...
        if (first_negative != not_a_dim)
        {
            ...
            for (dimension_type i = num_rows; i-- > 0; )
            {
                ...
                for (Row::const_iterator
                     j = s_pivot.begin(), j_end = s_pivot.end(); j != j_end;
                     ++j)
                {
                    if (j.index() != pj)
                    {
            ...
            for (dimension_type i = num_rows; i-- > 0; )
            {
                ...
                // Coefficient_traits::const_reference, because scale() may
                // silently
                // modify it.
                ...
                for (Row::const_iterator
                     j = t_pivot.begin(), j_end = t_pivot.end(); j != j_end;
                     ++j)
                {
        ...
        if (first_mixed != not_a_dim)
        {
...
#ifdef NOISY_PIP
                {
                    Linear_Expression expr = Linear_Expression(tautology.get(
                                                                   0));
                    dimension_type j = 1;
            ...
            PIP_Tree_Node * t_node = new PIP_Solution_Node(*this,
                                                           No_Constraints());
            ...
            // Modify *this in place to become the "false" version of current
            // node.
            ...
            // Compute the complement of the constraint used for the "true"
            // node.
            ...
            else if (f_node == nullptr)
            {
                ...
                if ((decision_node_p != nullptr) &&
                    (decision_node_p->false_child != nullptr))
                {
                    // Do NOT merge: create a new decision node.
                    PIP_Tree_Node * const parent
                        = new PIP_Decision_Node(t_node->get_owner(), nullptr,
                                                t_node);
                    // Previously wrapped 't_node' is now safe: release it
                    // and protect new 'parent' node from exception safety
                    // issues.
                    ...
                    // It is now safe to release previously wrapped parent
                    // pointer
                ...
                else
                {
                    ...
                    // It is now safe to release previously wrapped t_node
                    // pointer
                    // and return it to caller.
                    return safe_node.get_and_release();
                }
            ...
            if (!cs.empty())
            {
                ...
                parent = new PIP_Decision_Node(parent->get_owner(), nullptr,
                                               parent);
...
non_integer:
        ...
        if (cutting_strategy == PIP_Problem::CUTTING_STRATEGY_FIRST)
        {
            ...
            for (dimension_type k = 0; k < num_vars; ++k)
            {
                ...
                if ((pcount > 0) && ((best_i == not_a_dim) || (pcount <
                                                               best_pcount)))
                {
                    best_pcount = pcount;
                    best_i      = i;
            ...
            for (dimension_type k = 0; k < num_vars; ++k)
            {
                ...
                /*
                   Select row i if it is non integer AND
                    ...
                    - it has the same number of non-integer parameter
                       coefficients,
                                 ...
                                 const int            indent_level)
{
    ...
    if (generate_parametric_cut)
    {
        ...
        {
            // Here reuse_ap == false: the Artificial_Parameter does not exist
            // yet.
...
PIP_Tree_Node::print (std::ostream & s, const int indent) const
{
    ...
    const Variables_Set & pip_params    =
        get_owner()->parameter_space_dimensions();
    ...
    for (const PIP_Tree_Node * node = parent(); node != nullptr; node =
             node->parent())
    {
                               ...
                               const dimension_type first_art_dim) const
{
    ...
    PPL_ASSERT(true_child != 0);
    true_child->print_tree(s, indent + 1, pip_dim_is_param,
                           child_first_art_dim);
