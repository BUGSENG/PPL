...
{
    enum anonymous_enum { BASE, INTEGER, FRACTIONAL, EXPONENT
...
infinity:
        ...
        case 'I':
            ...
            return numer.neg_mantissa ? V_EQ_MINUS_INFINITY :
                   V_EQ_PLUS_INFINITY;
    }
            ...
            case BASE:
                ...
                if (c == '^')
                {
                    ...
                    for (std::string::const_iterator
                         i = numer.mantissa.begin(); i != numer.mantissa.end();
                         ++i)
                    {
                        numer.base = numer.base * 10 +
                                     static_cast<unsigned>(get_digit(*i, 10));
...
exponent:
                    ...
                    max_exp_div = static_cast<unsigned long>(l_max) /
                                  numer.base;
                    max_exp_rem = static_cast<int>(l_max %
                                                   static_cast<long>(numer.base));
            ...
            case EXPONENT:
                ...
                if (d >= 0)
                {
                    ...
                    if ((numer.exponent > max_exp_div)
                        || ((numer.exponent == max_exp_div) && (d >
                                                                max_exp_rem)))
                    {
                        return V_CVT_STR_UNK;
                    }

                    numer.exponent = 10 * numer.exponent + static_cast<unsigned
                                                                       long>(d);
                    break;
                }
...
ok:
    {
        ...
        sum_sign(numer.neg_exponent, numer.exponent,
                 neg, static_cast<unsigned long>(exponent_offset) *
                 exponent_offset_scale);
        return V_EQ;
    }
...
input_mpq (mpq_class & to, std::istream & is)
{
    ...
    if (denom_struct.base != 0)
    {
        ...
        if ((numer_struct.exponent != 0) || (denom_struct.exponent != 0))
        {
            ...
            {
                mpz_ui_pow_ui(z,
                              numer_struct.base_for_exponent,
                              numer_struct.exponent);
            ...
            {
                mpz_ui_pow_ui(z,
                              denom_struct.base_for_exponent,
                              denom_struct.exponent);
...
std::string float_mpq_to_string (mpq_class & q)
{
    ...
    if (decimals != 0)
    {
        ...
        if (decimals < len)
        {
            memmove(&buf[len - decimals + 1], &buf[len - decimals], decimals +
                    1);
            buf[len - decimals] = '.';
        }
        else
