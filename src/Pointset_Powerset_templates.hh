    ...
    : Base(), space_dim(y.space_dimension())
{
    ...
    for (typename Pointset_Powerset<QH>::const_iterator i = y.begin(),
         y_end = y.end(); i != y_end; ++i)
    {
        x.sequence.push_back(Determinate<PSET>(PSET(i->pointset(),
                                                    complexity)));
    }
                               ...
                               denominator)
{
    ...
    for (Sequence_iterator si = x.sequence.begin(),
         s_end = x.sequence.end(); si != s_end; ++si)
    {
        si->pointset().generalized_affine_preimage(var, relsym, expr,
                                                   denominator);
        x.reduced = false;
    }
                      ...
                      Pointset_Powerset<NNC_Polyhedron> & r)
{
    ...
    const Constraint      & neg_c = c.is_strict_inequality() ? (le <= 0) : (le <
                                                                            0);
