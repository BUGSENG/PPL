...
Bit_Row::Bit_Row (const Bit_Row & y, const Bit_Row & z)
{
    ...
    if (y_size < z_size)
    {
        ...
        mpz_init2(vec, static_cast<unsigned long>(z_size) *
                  PPL_BITS_PER_GMP_LIMB);
        union_helper(y, z);
    }
    else
    {
        PPL_ASSERT(static_cast<unsigned long>(y_size)
                   <= C_Integer<unsigned long>::max / PPL_BITS_PER_GMP_LIMB);
        mpz_init2(vec, static_cast<unsigned long>(y_size) *
                  PPL_BITS_PER_GMP_LIMB);
        union_helper(z, y);
    }
}
...
{
    return static_cast<memory_size_type>(vec[0]._mp_alloc) *
...
Bit_Row::union_assign (const Bit_Row & x, const Bit_Row & y)
{
    ...
    if (x_size < y_size)
    {
        PPL_ASSERT(static_cast<unsigned long>(y_size)
                   <= C_Integer<unsigned long>::max / PPL_BITS_PER_GMP_LIMB);
        mpz_realloc2(vec, static_cast<unsigned long>(y_size) *
                     PPL_BITS_PER_GMP_LIMB);
        union_helper(x, y);
    }
    else
    {
        PPL_ASSERT(static_cast<unsigned long>(x_size)
                   <= C_Integer<unsigned long>::max / PPL_BITS_PER_GMP_LIMB);
        mpz_realloc2(vec, static_cast<unsigned long>(x_size) *
                     PPL_BITS_PER_GMP_LIMB);
        union_helper(y, x);
    }
}
