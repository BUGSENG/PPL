    ...
    if (x.is_necessarily_closed())
    {
        if (!x.has_something_pending() && !y.has_something_pending())
        {
            ...
            if (x.constraints_are_minimized() && y.constraints_are_minimized())
            {
                ...
                const dimension_type x_num_equalities =
                    x.con_sys.num_equalities();
                ...
                //  - if there are no equalities, they have the same
                // constraints.
                    ...
                    for (dimension_type j = gs.num_rows(); j-- > 0; )
                    {
                        ...
                        switch (g.type())
                        {
                            case Generator::POINT:
                                ...
                                // If a point violates or saturates a strict
                                // inequality
                                // (when ignoring the epsilon coefficients) then
                                // it is
                            ...
                            case Generator::CLOSURE_POINT:
                                ...
                                // The generator is a ray or closure point:
                                // usual test.
                                if (sp_sign < 0)
                                {
                                    return false;
            ...
            if (sat_ci == sat_lines)
            {
                ...
                if (!found_eps_leq_one)
                {
                    ...
                    if (c.expression().all_homogeneous_terms_are_zero()
                        && (c.expression().inhomogeneous_term() +
                            c.epsilon_coefficient() == 0))
                    {
    ...
    for (dimension_type i = n_lines; i < gs_rows; )
    {
        ...
        if (g.is_point())
        {
            ...
            for (dimension_type j = n_lines; j < gs_rows; ++j)
            {
                ...
                if ((i != j) && g2.is_point() && subset_or_equal(sat[j],
                                                                 sat_gs_i))
                {
...
PPL::Polyhedron::BHZ09_C_poly_hull_assign_if_exact (const Polyhedron & y)
{
    ...
    {
        const bool included
            = y.relation_with(x_cs[i]).implies(
...
PPL::Polyhedron::BHZ09_NNC_poly_hull_assign_if_exact (const Polyhedron & y)
{
         ...
         i = x_points_non_redundant_in_y.next(i))
    {
        ...
        for (dimension_type j = y_cs_num_rows; j-- > 0; )
        {
            ...
            const int          sp_sign = Scalar_Products::reduced_sign(y_c,
                                                                       x_p);
    ...
    for (dimension_type i = x_cs_num_rows; i-- > 0; )
    {
        ...
        // Check condition 1.
        tmp_set.intersection_assign(x_non_points_non_redundant_in_y,
                                    saturators);
         ...
         i = y_points_non_redundant_in_x.next(i))
    {
        ...
        for (dimension_type j = x_cs_num_rows; j-- > 0; )
        {
            ...
            const int          sp_sign = Scalar_Products::reduced_sign(x_c,
                                                                       y_p);
    ...
    for (dimension_type i = y_cs_num_rows; i-- > 0; )
    {
        ...
        // Check condition 1.
        tmp_set.intersection_assign(y_non_points_non_redundant_in_x,
                                    saturators);
    ...
    for (dimension_type i = y_cs_num_rows; i-- > 0; )
    {
        ...
        if (y_c.is_strict_inequality())
        {
            for (dimension_type j = x_gs_condition_3.first();
                 j != C_Integer<unsigned long>::max; j = x_gs_condition_3.next(
                     j))
            {
    ...
    for (dimension_type i = x_cs_num_rows; i-- > 0; )
    {
        if (x_cs[i].is_strict_inequality())
        {
            ...
            for (dimension_type j = y_gs_condition_3.first();
                 j != C_Integer<unsigned long>::max; j = y_gs_condition_3.next(
                     j))
            {
    ...
    for (dimension_type i = x_gs_num_rows; i-- > 0; )
    {
        ...
        for (dimension_type j = y_gs_num_rows; j-- > 0; )
        {
            ...
            const bool illegal_ray
                = (mid_g.expr.inhomogeneous_term() == 0 &&
                   mid_g.expr.all_homogeneous_terms_are_zero());
...
PPL::Polyhedron::positive_time_elapse_assign_impl (const Polyhedron & y)
{
    ...
    // Remove all points from new_gs and put them in 'x_points_gs' for later
    // use.
    ...
    for (dimension_type i = gs_num_rows; i-- > 0; )
    {
        ...
        switch (g.type())
        {
            case Generator::POINT:
                ...
                // However, for each point there is a corresponding closure
                // point in "gs".
                ...
                for (dimension_type j = x_points_gs.sys.num_rows(); j-- > 0; )
                {
                    ...
                    Coefficient new_divisor = g.expr.inhomogeneous_term() *
                                                 ...
                                                 const char         * reason)
{
    ...
    return PPL::check_space_dimension_overflow(dim, max, domain, method,
                                               reason);
}
...
PPL::Polyhedron::check_space_dimension_overflow (const dimension_type dim,
                                                 ...
                                                 const char         * reason)
{
    return check_space_dimension_overflow(dim,
