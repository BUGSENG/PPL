         ...
         gs_end = gs.end(); i != gs_end; ++i)
    {
        switch (i->type())
        {
            ...
            case Generator::RAY:
                ...
                ++num_rays_null_coord[i->expression().num_zeroes(1, space_dim +
                                                                 1)];
            break;
...
PPL::BHRZ03_Certificate::compare (const Polyhedron & ph) const
{
         ...
         gs_end = gs.end(); i != gs_end; ++i)
    {
        if (i->is_ray())
        {
            ++ph_num_rays_null_coord[i->expression().num_zeroes(1, space_dim +
                                                                1)];
        }
    }
    ...
    for (dimension_type i = 0; i < space_dim; ++i)
    {
        ...
        {
            return (ph_num_rays_null_coord[i] < num_rays_null_coord[i]) ? 1 :
                   -1;
        }
    }
