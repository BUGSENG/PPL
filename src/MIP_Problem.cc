        ...
        case PARTIALLY_SATISFIABLE:
            {
                ...
                Generator          g = point();
                {
                    ...
                    {
                        x.status = UNSATISFIABLE;
                        // NOTE: `relaxed' destroyed: relaxation automatically
                        // reset.
                        return UNFEASIBLE_MIP_PROBLEM;
                    }
                ...
                switch (return_value)
                {
                    ...
                    case UNBOUNDED_MIP_PROBLEM:
                        x.status = UNBOUNDED;
                        // A feasible point has been set in `solve_mip()', so
                        // that
                     ...
                     std::deque<bool> & is_remergeable_variable) const
{
    ...
    for (dimension_type i = cs_num_rows; i-- > first_pending_constraint; )
    {
        ...
        const bool found_a_nonzero_coeff = (nonzero_coeff_column_index !=
                                            cs_i_end);
        ...
        if (found_many_nonzero_coeffs)
        {
            if (cs_i.is_inequality())
            {
                ...
                // `is_satisfied()' with the generator `point()'?  If so, the
                // following
                // code works even if we do not have a feasible point.
                // Check for satisfiability of the inequality. This can be done
                // if we
        ...
        else
        {
            ...
            const dimension_type nonzero_var_index =
                nonzero_coeff_column_index - 1;

            const int sgn_a = sgn(cs_i.coefficient(Variable(
                                                       nonzero_var_index)));
            ...
            else if (sgn_a > 0)
            {
                if (!is_nonnegative_variable[nonzero_var_index])
                {
                    ...
                    {
                        // Remember to merge back the positive and negative
                        // parts.
...
PPL::MIP_Problem::process_pending_constraints ()
{
    ...
    {
        const dimension_type space_diff = external_space_dim -
                                          internal_space_dim;
    ...
    const dimension_type num_satisfied_inequalities
        = static_cast<dimension_type>(std::count(
                                          is_satisfied_inequality.begin(),
         ...
         i = input_cs.size() - first_pending_constraint; i-- > 0; )
    {
        ...
        if (c.is_inequality())
        {
            ...
            // If the constraint is already satisfied, we will not use
            // artificial
    ...
    for (dimension_type i = 0; i < unfeasible_tableau_rows_size; ++i)
    {
        tableau[unfeasible_tableau_rows[i]].insert(artificial_index,
                                                   Coefficient_one());
        cost_itr = working_cost.insert(cost_itr,
                                       artificial_index);
        *cost_itr                        = -1;
    ...
    // Express the problem in terms of the variables in base.
    {
        ...
        for (dimension_type i = tableau_num_rows; i-- > 0; )
        {
            ...
            if ((itr != working_cost.end()) && (itr.index() == base[i]) &&
                (*itr != 0))
            {
...
PPL::MIP_Problem::steepest_edge_float_entering_index () const
{
    ...
    const int      cost_sign      = sgn(working_cost.get(working_cost.size() -
                                                         1));
    ...
    for (dimension_type i = tableau_num_rows; i-- > 0; )
    {
        ...
        Row::const_iterator j =
            tableau_i.begin();
        Row::const_iterator j_end =
            tableau_i.end();
    ...
    for (dimension_type j = tableau_num_columns - 1; j-- > 1; )
    {
        ...
        if (sgn(cost_j) == cost_sign)
        {
            ...
            // Due to our integer implementation, the `1' term in the
            // denominator
            // of the original formula has to be replaced by
            // `squared_lcm_basis'.
            ...
            for (dimension_type i = tableau_num_rows; i-- > 0; )
            {
                ...
                if (tableau_ij != 0)
                {
                    ...
                    float_tableau_value /= float_tableau_denom;
                    challenger_denom    += float_tableau_value *
                                           float_tableau_value;
                }
            }
...
PPL::MIP_Problem::steepest_edge_exact_entering_index () const
{
    ...
    std::vector<Coefficient> norm_factor(tableau_num_rows);
    {
        ...
        for (dimension_type i = tableau_num_rows; i-- > 0; )
        {
            exact_div_assign(norm_factor[i], lcm_basis, tableau[i].get(
                                 base[i]));
        }
...
#    else
                ...
                // The test against 0 gives rise to a consistent speed up in the
                // dense
                // implementation: see
    ...
    for (dimension_type j = tableau.num_columns() - 1; j-- > 1; )
    {
        ...
        if (sgn(cost_j) == cost_sign)
        {
            ...
            for (dimension_type i = tableau_num_rows; i-- > 0; )
            {
                ...
                {
                    scalar_value = tableau_ij * norm_factor[i];
                    add_mul_assign(challenger_denom, scalar_value,
                                   scalar_value);
                }
            }
...
PPL::MIP_Problem::textbook_entering_index () const
{
    ...
    const int            cost_sign       = sgn(working_cost.get(
                                                   cost_sign_index));
                                  ...
                                  const dimension_type k)
{
    ...
    neg_assign(normalized_y_k);
    Parma_Polyhedra_Library::linear_combine(x, y, normalized_y_k,
                                            normalized_x_k);
                         ...
                         const dimension_type exiting_base_index)
{
    ...
    for (dimension_type i = tableau.num_rows(); i-- > 0; )
    {
        ...
        if ((i != exiting_base_index) && (tableau_i.get(entering_var_index) !=
                                          0))
        {
...
::get_exiting_base_index (const dimension_type entering_var_index) const
{
    ...
    {
        const Row                         & t_i  = tableau[i];
        Coefficient_traits::const_reference t_ie = t_i.get(
            entering_var_index);
...
PPL::MIP_Problem::second_phase ()
{
    ...
    // Express the old cost function in terms of the computed base.
    {
        ...
        for (dimension_type i = tableau.num_rows(); i-- > 0; )
        {
            ...
            if ((itr != working_cost.end()) && (itr.index() == base_i) &&
                (*itr != 0))
            {
        ...
        case PARTIALLY_SATISFIABLE:
            {
                ...
                // This code tries to handle the case that happens if the
                // tableau is
                // empty, so it must be initialized.
                if (tableau.num_columns() == 0)
                {
                    // Add two columns, the first that handles the inhomogeneous
                    // term and
                    ...
                    // The internal data structures are ready, so prepare for
                    // more
                                             ...
                                             branching_index)
{
    ...
    for (dimension_type i = input_cs_num_rows; i-- > 0; )
    {
        ...
        // TODO: This can be optimized more, exploiting the (possible)
        // sparseness of input_cs, if the size of candidate_variables is
        // expected
...
PPL::MIP_Problem::OK () const
{
    ...
    if ((status != UNSATISFIABLE) && initialized)
    {
        ...
        if (!i_variables.empty())
        {
            ...
            // TODO: This can be optimized more, exploiting the (possible)
            // sparseness of last_generator, if the size of i_variables is
            // expected
            // to be greater than the number of nonzeroes in last_generator in
            // most
            // cases.
        ...
        // The number of columns in the tableau and working_cost should be
        // equal.
...

        {
            ...
            for (dimension_type j = tableau_num_rows; j-- > 0; )
            {
                ...
                for ( ; i != i_end && itr != itr_end; ++i)
                {
                    ...
                    if ((i->second != j) && (itr.index() == i->first) &&
                        (*itr != 0))
                    {
#ifndef NDEBUG
                        cerr <<
