...
test02 ()
{
    ...
    box.bounded_affine_preimage(B, 3 * E - 5 * D + A - 3 * B, 4 * A - 2 * C + 3,
                                -3);
    ph.bounded_affine_preimage(B, 3 * E - 5 * D + A - 3 * B, 4 * A - 2 * C + 3,
                               -3);
...
test16 ()
{
    ...
    try
    {
        ...
        // Box::bounded_affine_preimage(v, lb_expr, ub_epxr, d): it is illegal
        // to
