...
test04 ()
{
    ...
    bool ok = box1.contains(precise_result) && good_enough_result.contains(box1)
              && box2.contains(precise_result) && good_enough_result.contains(
        box2)
              && box3.contains(precise_result) && good_enough_result.contains(
        box3);

    print_constraints(box1, "*** box.wrap_assign(..., OVERFLOW_WRAPS) ***");
    print_constraints(box2, "*** box.wrap_assign(..., OVERFLOW_UNDEFINED) ***");
    print_constraints(box3,
                      "*** box.wrap_assign(..., OVERFLOW_IMPOSSIBLE) ***");
