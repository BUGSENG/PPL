                               ...
                               Generator_System        & gs)
{
    ...
    {
        gs.insert(closure_point(weight_center + half_diagonal * Variable(
                                    axis)));
        gs.insert(closure_point(weight_center - half_diagonal * Variable(
                                    axis)));
    }
...
test01 ()
{
    ...
    for (dimension_type dims = first_dim; dims <= last_dim; dims++)
    {
        for (int percentage = 25; percentage <= 50; percentage += 25)
        {
            ...
            known_result = standard_cardinalities.find(make_pair(dims,
                                                                 percentage));
            ...
            // Check if there is a known result.
            known_result = enhanced_cardinalities.find(make_pair(dims,
                                                                 percentage));
