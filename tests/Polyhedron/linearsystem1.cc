    ...
    for (dimension_type rowi = 0; rowi < ROWS; ++rowi)
    {
        ...
        for (dimension_type col = 0; col < COLS; ++col)
        {
                ...
                && (c == std::numeric_limits<Coefficient>::min()))
            {
                ...
                // Thus, it would cause a positive overflow during the
                // normalization
                // of the Linear_Row, because the GCD computation will try to
                // negate
                // such a coefficient.
        ...
        nout << "Linear_System<Constraint>::ascii_dump/load test failed." <<
            endl
