...
test02 ()
{
    ...
    if (ok)
    {
        ...
        if ((root->child_node(true) == nullptr) || (root->child_node(false) !=
                                                    nullptr))
        {
            return false;
        }

        const PIP_Decision_Node * t_child = root->child_node(
            true)->as_decision();
        ...
        if ((t_child->child_node(true) == nullptr) || (t_child->child_node(
                                                           false) == nullptr))
        {
            return false;
        }
...
test22 ()
{
    // Same problem as test02, but incrementally adding two parameter
    // constraints
