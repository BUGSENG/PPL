...
test02 ()
{
    ...
    Pointset_Powerset<NNC_Polyhedron>::const_iterator nnc_i =
        nnc_pps1.begin();
    NNC_Polyhedron nnc_phi =
        nnc_i->pointset();
...
test06 ()
{
    ...
    Pointset_Powerset<TBD_Shape> pps1(ph1,
    ...
    ok = check_result(bdsi1, known_bdsi1, "3.98e-8", "3.98e-8", "3.98e-8") &&
         ok;
...
test10 ()
{
    ...
    // With the polynomial complexity, implied equalities are not found.
    Pointset_Powerset<TOctagonal_Shape> pps1(ph1,
                                             POLYNOMIAL_COMPLEXITY);
